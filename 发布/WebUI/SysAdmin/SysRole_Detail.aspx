<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysRole_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysRoleDetail" %>
...
<h4 class="txt_title" style="float: left">查看后台角色</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="SysRole_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="角色名" colspan ="3"><span class="col_name">角色名：</span><%=HttpUtility.HtmlEncode(entity.RoleName.ToStr()) %></td><td  title ="后台类型" colspan ="3"><span class="col_name">后台类型：</span><%=HttpUtility.HtmlEncode(entity.SysType.ToStr()) %></td> </tr>
<tr> <td  title ="有权功能列表" colspan ="3"><span class="col_name">有权功能列表：</span><%=HttpUtility.HtmlEncode(entity.UserPowerList.ToStr()) %></td><td  title ="修改人员名称" colspan ="3"><span class="col_name">修改人员名称：</span><%=HttpUtility.HtmlEncode(entity.EditUserName.ToStr()) %></td> </tr>
<tr> <td  title ="修改时间" colspan ="6"><span class="col_name">修改时间：</span><%=HttpUtility.HtmlEncode(entity.EditTime.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
