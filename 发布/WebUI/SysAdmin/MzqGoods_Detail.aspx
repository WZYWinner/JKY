<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MzqGoods_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.MzqGoodsDetail" %>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="MzqGoods_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">卖家：</span><%=HttpUtility.HtmlEncode(entity.MzqGoodsSeller.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">商品名称：</span><%=HttpUtility.HtmlEncode(entity.MzqGoodsName.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品价格：</span><%=HttpUtility.HtmlEncode(entity.MzqGoodsPrice.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">商品图片：</span><input type="hidden" name="hidden_fileMzqGoodsPic" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileMzqGoodsPic" runat="server" /></td> </tr>
</tbody>
</table>
</div>
</div>
