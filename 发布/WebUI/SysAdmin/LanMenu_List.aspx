<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LanMenuList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LanMenuList" ValidateRequest="false" %>
...
// 绑定事件
url='LanMenu_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="">广告图片</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("LanMenuId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("LanMenuId") %>' />
 </td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("LanMenuName").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("LanMenuImg").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("LanMenuPostImg").ToStrCn()) %> </span></td>
 </tr>
 </ItemTemplate>
 <EmptyDataTemplate>
 <!--**** 没有数据的情况 ****-->
...
<a class="btn btn-primary" href="LanMenu_Upload.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
