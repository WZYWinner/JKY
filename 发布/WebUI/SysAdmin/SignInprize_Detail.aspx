<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SignInprize_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SignInprizeDetail" %>
...
<h4 class="txt_title" style="float: left">查看签到有奖</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="SignInprize_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">用户ID：</span><%=HttpUtility.HtmlEncode(entity.UserId.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">积分等级：</span><%=HttpUtility.HtmlEncode(entity.SignGrade.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">总积分：</span><%=HttpUtility.HtmlEncode(entity.SignCount.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">连续签到天数：</span><%=HttpUtility.HtmlEncode(entity.ContinuityDay.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">签到总计天数：</span><%=HttpUtility.HtmlEncode(entity.SignDayCount.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">积分奖励：</span><%=HttpUtility.HtmlEncode(entity.Integral.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">签到时间：</span><%=HttpUtility.HtmlEncode(entity.SignTime.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">最近修改人员：</span><%=HttpUtility.HtmlEncode(entity.EditUser.ToStr()) %></td> </tr>
