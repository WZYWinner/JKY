<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LxcUser_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LxcUserDetail" %>
...
<h4 class="txt_title" style="float: left">查看用户登录</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LxcUser_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">账号：</span><%=HttpUtility.HtmlEncode(entity.UserName.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">密码：</span><%=HttpUtility.HtmlEncode(entity.UserPasswd.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="6"><span class="col_name">账户注册时间：</span><%=HttpUtility.HtmlEncode(entity.UserTime.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
