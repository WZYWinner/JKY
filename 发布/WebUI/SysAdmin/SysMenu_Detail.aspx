<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysMenu_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysMenuDetail" %>
...
<h4 class="txt_title" style="float: left">查看系统菜单</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="SysMenu_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="中文名" colspan ="3"><span class="col_name">中文名：</span><%=HttpUtility.HtmlEncode(entity.CnName.ToStr()) %></td><td  title ="权重" colspan ="3"><span class="col_name">权重：</span><%=HttpUtility.HtmlEncode(entity.PowerIndex.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
