<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LanUser_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LanUserDetail" %>
...
<h4 class="txt_title" style="float: left">查看LAN用户</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LanUser_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">用户名：</span><%=HttpUtility.HtmlEncode(entity.LanUserName.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">用户密码：</span><%=HttpUtility.HtmlEncode(entity.LanUserPwd.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">真实姓名：</span><%=HttpUtility.HtmlEncode(entity.LanUserRealname.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">联系电话：</span><%=HttpUtility.HtmlEncode(entity.LanUserPhone.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">头像：</span><input type="hidden" name="hidden_fileLanUserImg" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileLanUserImg" runat="server" /></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">性别：</span><%=HttpUtility.HtmlEncode(entity.LanUserSex.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">注册时间：</span><%=HttpUtility.HtmlEncode(entity.LanUserAddtime.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
