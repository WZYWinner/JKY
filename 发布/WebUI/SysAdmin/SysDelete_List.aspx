<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysDeleteList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysDeleteList" ValidateRequest="false" %>
...
// 绑定事件
url='SysDelete_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="">删除时间</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("DelId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("DelId") %>' />
 </td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("DataId").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("DataType").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("DataJson").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("DelTime").ToStrCn()) %> </span></td>
 </tr>
 </ItemTemplate>
 <EmptyDataTemplate>
 <!--**** 没有数据的情况 ****-->
...
<a class="btn btn-primary" href="SysDelete_Upload.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
