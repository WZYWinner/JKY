<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LxcGoods_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LxcGoodsDetail" %>
...
<h4 class="txt_title" style="float: left">查看商城商品</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LxcGoods_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品名称：</span><%=HttpUtility.HtmlEncode(entity.LxcGoodsName.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">商品价格：</span><%=HttpUtility.HtmlEncode(entity.LxcGoodsPrice.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">商品图片：</span><input type="hidden" name="hidden_fileLxcGoodsPic" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileLxcGoodsPic" runat="server" /></td><td  title ="" colspan ="2"><span class="col_name">商品数量：</span><%=HttpUtility.HtmlEncode(entity.LxcGoodsNum.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">商品子图片：</span><input type="hidden" name="hidden_filesLxcGoodsPics" class=""/><UC:UC_Plupload IsPicMode="true" PageType="只读" ID="filesLxcGoodsPics" runat="server" /></td> </tr>
</tbody>
</table>
</div>
</div>
