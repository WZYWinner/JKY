<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoMerchantList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.GsfAdmin.GuoMerchantList" ValidateRequest="false" %>
...
// 绑定事件
url='GuoMerchant_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">商家名字</th>
<th style="    min-width: 80px;">商家图片</th>
<th style="    min-width: 80px;">商家配送</th>
<th style="    min-width: 80px;">标签</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("MerchantId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("MerchantId") %>' />
 </td>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("MerchantImg").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("MerchantDelivery").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="GuoMerchant_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
