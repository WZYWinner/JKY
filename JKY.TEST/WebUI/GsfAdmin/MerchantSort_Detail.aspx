<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MerchantSort_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.GsfAdmin.MerchantSortDetail" %>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="MerchantSort_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="分类名字" colspan ="3"><span class="col_name">分类名字：</span><%=HttpUtility.HtmlEncode(entity.SortName.ToStr()) %></td><td  title ="分类图片" colspan ="3"><span class="col_name">分类图片：</span><%=HttpUtility.HtmlEncode(entity.SortImg.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
