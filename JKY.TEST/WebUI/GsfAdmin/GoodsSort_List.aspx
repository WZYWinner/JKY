<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsSortList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.GsfAdmin.GoodsSortList" ValidateRequest="false" %>
...
// 绑定事件
url='GoodsSort_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">商品分类名字</th>
...
<a class="btn btn-primary" href="GoodsSort_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
