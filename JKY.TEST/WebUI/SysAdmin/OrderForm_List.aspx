<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OrderFormList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.OrderFormList" ValidateRequest="false" %>
...
// 绑定事件
url='OrderForm_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">订单总价</th>
<th style="    min-width: 80px;">红包优惠</th>
<th style="    min-width: 80px;">实付价格</th>
<th style="    min-width: 80px;">收货人电话</th>
<th style="    min-width: 80px;">收货地址</th>
<th style="    min-width: 80px;">接单商家</th>
<th style="    min-width: 80px;">订单备注</th>
<th style="    min-width: 80px;">订单状态</th>
 <th class="o_title2" style="width: 150px;"> 操作 </th>
 </tr>
 </thead>
 <tbody>
 <wc:Repeater ID="UiDataList" runat="server">
 <ItemTemplate>
 <tr dateid="<%#Eval("OrderId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("OrderId") %>' />
 </td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderUser").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderTime").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderTotalPrice").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderDiscounts").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderRealPeice").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderTel").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderAddress").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderMerchants").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderNote").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("OrderStatus").ToStrCn()) %> </span></td>
 <td>
 <%# Eval("_CustomCode['LIST']") %>
 </td>
 </tr>
 </ItemTemplate>
 <EmptyDataTemplate>
...
<a class="btn btn-primary" href="OrderForm_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
