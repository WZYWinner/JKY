<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoOrderFormList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.GuoOrderFormList" ValidateRequest="false" %>
...
// 绑定事件
url='GuoOrderForm_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<a class="btn btn-primary" href="GuoOrderForm_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
