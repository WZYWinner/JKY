<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysConfigList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysConfigList" ValidateRequest="false" %>
...
// 绑定事件
url='SysConfig_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">名称</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("ConfigId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("ConfigId") %>' />
 </td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("TypeId").ToStrCn()) %> </span></td>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("Code").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="SysConfig_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
