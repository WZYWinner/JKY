<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LiuAddress_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LiuAddressDetail" %>
...
<h4 class="txt_title" style="float: left">查看用户地址</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LiuAddress_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="省份" colspan ="3"><span class="col_name">省份：</span><%=HttpUtility.HtmlEncode(entity.Province.ToStr()) %></td><td  title ="收货地址" colspan ="3"><span class="col_name">收货地址：</span><%=HttpUtility.HtmlEncode(entity.Address.ToStr()) %></td> </tr>
<tr> <td  title ="收货人" colspan ="2"><span class="col_name">收货人：</span><%=HttpUtility.HtmlEncode(entity.Receiver.ToStr()) %></td><td  title ="收货人电话" colspan ="2"><span class="col_name">收货人电话：</span><%=HttpUtility.HtmlEncode(entity.Telephone.ToStr()) %></td><td  title ="标签" colspan ="2"><span class="col_name">标签：</span><%=HttpUtility.HtmlEncode(entity.Laber.ToStr()) %></td> </tr>
<tr> <td  title ="用户ID" colspan ="6"><span class="col_name">用户ID：</span><%=HttpUtility.HtmlEncode(entity.UserId.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
