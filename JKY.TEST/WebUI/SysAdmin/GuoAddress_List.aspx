<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoAddressList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.GuoAddressList" ValidateRequest="false" %>
...
// 绑定事件
url='GuoAddress_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">门牌号</th>
...
<th style="    min-width: 80px;">标签</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("GuoAddressId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("GuoAddressId") %>' />
 </td>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("HouseNumber").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="GuoAddress_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
