<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CarouselTwoList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.CarouselTwoList" ValidateRequest="false" %>
...
// 绑定事件
url='CarouselTwo_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<a class="btn btn-primary" href="CarouselTwo_Upload.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
