<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MerchantClassify_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.MerchantClassifyDetail" %>
...
<h4 class="txt_title" style="float: left">查看商家分类</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="MerchantClassify_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="商家分类ID" colspan ="2"><span class="col_name">商家分类ID：</span><%=HttpUtility.HtmlEncode(entity.MerchantSortId.ToStr()) %></td><td  title ="商家分类名" colspan ="2"><span class="col_name">商家分类名：</span><%=HttpUtility.HtmlEncode(entity.SortName.ToStr()) %></td><td  title ="商家分类LOGO" colspan ="2"><span class="col_name">商家分类LOGO：</span><%=HttpUtility.HtmlEncode(entity.SortImg.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
