<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UserInfo_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.UserInfoDetail" %>
...
<h4 class="txt_title" style="float: left">查看用户信息</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="UserInfo_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">用户名：</span><%=HttpUtility.HtmlEncode(entity.Name.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">密码：</span><%=HttpUtility.HtmlEncode(entity.Pwd.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">头像：</span><input type="hidden" name="hidden_fileHead" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileHead" runat="server" /></td><td  title ="" colspan ="2"><span class="col_name">密保手机号码：</span><%=HttpUtility.HtmlEncode(entity.Tel.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">经验：</span><%=HttpUtility.HtmlEncode(entity.Experience.ToStr()) %></td> </tr>
