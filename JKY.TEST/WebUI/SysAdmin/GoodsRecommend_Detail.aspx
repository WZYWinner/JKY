<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsRecommend_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.GoodsRecommendDetail" %>
...
<h4 class="txt_title" style="float: left">查看商品推荐</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="GoodsRecommend_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品ID：</span><%=HttpUtility.HtmlEncode(entity.GoodsId.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">商品名称：</span><%=HttpUtility.HtmlEncode(entity.GoodsName.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">商品图片：</span><input type="hidden" name="hidden_fileGoodsPic" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileGoodsPic" runat="server" /></td><td  title ="" colspan ="2"><span class="col_name">推荐时长：</span><%=HttpUtility.HtmlEncode(entity.Length.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">推荐位置：</span><%=HttpUtility.HtmlEncode(entity.Position.ToStr()) %></td> </tr>
