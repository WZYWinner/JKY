<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysUserList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysUserList" ValidateRequest="false" %>
...
// 绑定事件
url='SysUser_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">用户帐号</th>
<th style="    min-width: 80px;">角色名</th>
<th style="    min-width: 80px;">后台类型</th>
<th style="    min-width: 80px;">真实姓名</th>
<th style="    min-width: 80px;">密保手机号码</th>
 </tr>
 </thead>
 <tbody>
 <wc:Repeater ID="UiDataList" runat="server">
 <ItemTemplate>
 <tr dateid="<%#Eval("SysUserId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("SysUserId") %>' />
 </td>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("RoleId").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="#DEA88B,#82D1AF,#7EB9EB" _Key="SysAdmin,Company,Agent" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("SysType").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("RealName").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="SysUser_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
