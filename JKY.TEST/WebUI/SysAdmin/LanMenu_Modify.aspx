<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="LanMenu_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.LanMenuModify" %>
...
<h4 class="txt_title" style="float: left">编辑菜单分类</h4>
...
<a class="btn btn-primary" href="javascript:window.location.href='LanMenu_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">菜单名称：</span><asp:TextBox ID="tbLanMenuName" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td><td  title ="" colspan ="3"><span class="col_name">菜单图标：</span><input type="hidden" name="hidden_fileLanMenuImg" class=""/><UC:UC_PluploadSingle PageType ="可写" ID="fileLanMenuImg" runat="server" /><b></b></td> </tr>
<tr> <td  title ="" colspan ="6"><span class="col_name">广告图片：</span><input type="hidden" name="hidden_fileLanMenuPostImg" class=""/><UC:UC_PluploadSingle PageType ="可写" ID="fileLanMenuPostImg" runat="server" /><b></b></td> </tr>
</tbody>
</table>
</div>
</div>
