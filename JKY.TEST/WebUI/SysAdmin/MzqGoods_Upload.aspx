<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="MzqGoods_Upload.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.MzqGoodsUpload" %>
...
<div class="btn_left" style="overflow: hidden; float: left; margin-left: 30px"><a class="btn btn-primary" href="javascript:window.location.href='MzqGoods_List.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a></div>
</small>
</div>
