<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysMenuList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysMenuList" ValidateRequest="false" %>
...
// 绑定事件
url='SysMenu_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="">权重</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("MenuId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("MenuId") %>' />
 </td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("CnName").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="SysMenu_Upload.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
