<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WangUserList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.WangUserList" ValidateRequest="false" %>
...
// 绑定事件
url='WangUser_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">密码</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("WangUserId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("WangUserId") %>' />
 </td>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("Password").ToStrCn()) %> </span></td>
 </tr>
 </ItemTemplate>
 <EmptyDataTemplate>
 <!--**** 没有数据的情况 ****-->
...
<a class="btn btn-primary" href="WangUser_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
