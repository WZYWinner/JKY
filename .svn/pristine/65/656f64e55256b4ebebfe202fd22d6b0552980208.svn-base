...
{
public static partial class CarouselTwoBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(CarouselTwoDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = CarouselTwoDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(CarouselTwoDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = CarouselTwoDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = CarouselTwoDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(CarouselTwoDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.Id.ToStr()))
return ret;
ret = CarouselTwoDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static CarouselTwoDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = CarouselTwoDAL.Select(0, id,0);
////添加附加按钮代码
...
public static CarouselTwoDALEntity GetCustomCode( CarouselTwoDALEntity entity)
{
...
public static List<CarouselTwoDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<CarouselTwoDALEntity>();
count = 0;
//查询实体赋值
var parm = new CarouselTwoQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = CarouselTwoDAL.Count(0, parm,0);
//数据库查询
list = CarouselTwoDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<CarouselTwoDALEntity>();
for (int i = 0; i < list.Count; i++)
{
CarouselTwoDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<CarouselTwoDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 CarouselTwoDALEntity[] list = CarouselTwoDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (CarouselTwoDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 CarouselTwoDALEntity oldEntity = null;
 if (item.Id != null)
 oldEntity = CarouselTwoBLL.GetEntity(item.Id.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( CarouselTwoDALEntity[] input)
{
DataTable result = new DataTable();
List<CarouselTwoDALEntity> list = new List<CarouselTwoDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
CarouselTwoDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
int i = 0;
////填充DataTable的数据
foreach (CarouselTwoDALEntity item in list)
{
result.Rows.Add(
item.Seller,
item.AdPic,
