<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoGoodsList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.GsfAdmin.GuoGoodsList" ValidateRequest="false" %>
...
// 绑定事件
url='GuoGoods_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">商品图</th>
<th style="    min-width: 80px;">标签</th>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("GoodsImg").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="GuoGoods_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
