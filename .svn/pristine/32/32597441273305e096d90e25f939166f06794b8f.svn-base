<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LxcUserList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LxcUserList" ValidateRequest="false" %>
...
// 绑定事件
url='LxcUser_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="">账户注册时间</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("LxcUserId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("LxcUserId") %>' />
 </td>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("UserPasswd").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("UserTime").ToStrCn()) %> </span></td>
 </tr>
 </ItemTemplate>
 <EmptyDataTemplate>
 <!--**** 没有数据的情况 ****-->
...
<a class="btn btn-primary" href="LxcUser_Upload.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
