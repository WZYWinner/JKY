<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysConfig_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysConfigDetail" %>
<%@ Import Namespace="Yaohuasoft.Framework.Library" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
<title>通用配置信息详情 | </title>
...
<h4 class="txt_title" style="float: left">查看通用配置信息</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="SysConfig_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="数据类型" colspan ="3"><span class="col_name">数据类型：</span><%=HttpUtility.HtmlEncode(entity.TypeId.ToStr()) %></td><td  title ="权重" colspan ="3"><span class="col_name">权重：</span><%=HttpUtility.HtmlEncode(entity.PowerIndex.ToStr()) %></td> </tr>
<tr> <td  title ="代码" colspan ="2"><span class="col_name">代码：</span><%=HttpUtility.HtmlEncode(entity.Code.ToStr()) %></td><td  title ="名称" colspan ="2"><span class="col_name">名称：</span><%=HttpUtility.HtmlEncode(entity.Name.ToStr()) %></td><td  title ="描述" colspan ="2"><span class="col_name">描述：</span><%=HttpUtility.HtmlEncode(entity.Descript.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
