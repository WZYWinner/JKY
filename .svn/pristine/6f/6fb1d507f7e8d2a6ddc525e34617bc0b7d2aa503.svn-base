<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysDelete_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysDeleteDetail" %>
...
<h4 class="txt_title" style="float: left">查看数据回收站</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="SysDelete_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="删除ID" colspan ="2"><span class="col_name">删除ID：</span><%=HttpUtility.HtmlEncode(entity.DelId.ToStr()) %></td><td  title ="源数据ID" colspan ="2"><span class="col_name">源数据ID：</span><%=HttpUtility.HtmlEncode(entity.DataId.ToStr()) %></td><td  title ="数据类型" colspan ="2"><span class="col_name">数据类型：</span><%=HttpUtility.HtmlEncode(entity.DataType.ToStr()) %></td> </tr>
<tr> <td  title ="数据内容JSON" colspan ="3"><span class="col_name">数据内容JSON：</span><%=HttpUtility.HtmlEncode(entity.DataJson.ToStr()) %></td><td  title ="删除时间" colspan ="3"><span class="col_name">删除时间：</span><%=HttpUtility.HtmlEncode(entity.DelTime.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
