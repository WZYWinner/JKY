<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LxcAddress_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LxcAddressDetail" %>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LxcAddress_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">用户名：</span><%=HttpUtility.HtmlEncode(entity.LxcAddressName.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">手机号：</span><%=HttpUtility.HtmlEncode(entity.LxcAddressPhone.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">收货地址：</span><%=HttpUtility.HtmlEncode(entity.LxcAddressAdres.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">收货状态：</span><%=HttpUtility.HtmlEncode(entity.LxcAddressStatus.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
