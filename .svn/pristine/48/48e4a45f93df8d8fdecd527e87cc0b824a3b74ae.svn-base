<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="SysDelete_Upload.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.SysDeleteUpload" %>
<%@ Import Namespace="Yaohuasoft.Framework.Library" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
<title>EXCEL导入数据回收站 | </title>
...
<h4 class="txt_title" style="float: left">导入数据回收站</h4>
<div class="btn_left" style="overflow: hidden; float: left; margin-left: 30px"><a class="btn btn-primary" href="javascript:window.location.href='SysDelete_List.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a></div>
</small>
</div>
