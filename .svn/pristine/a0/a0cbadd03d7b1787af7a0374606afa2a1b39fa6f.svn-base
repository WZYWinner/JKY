<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LanAddress_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LanAddressDetail" %>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LanAddress_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">用户ID：</span><%=HttpUtility.HtmlEncode(entity.SysUserId.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">收货人：</span><%=HttpUtility.HtmlEncode(entity.LanAddressUsername.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">收货人电话：</span><%=HttpUtility.HtmlEncode(entity.LanAddressPhone.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">收货地址：</span><%=HttpUtility.HtmlEncode(entity.LanAddressName.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
