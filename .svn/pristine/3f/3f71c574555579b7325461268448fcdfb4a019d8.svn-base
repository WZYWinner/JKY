...
{
public static partial class GoodsSortBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GoodsSortDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GoodsSortDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GoodsSortDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GoodsSortDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = GoodsSortDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GoodsSortDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.GoodsSortId.ToStr()))
return ret;
ret = GoodsSortDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GoodsSortDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GoodsSortDAL.Select(0, id,0);
////添加附加按钮代码
...
public static GoodsSortDALEntity GetCustomCode( GoodsSortDALEntity entity)
{
...
public static List<GoodsSortDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GoodsSortDALEntity>();
count = 0;
//查询实体赋值
var parm = new GoodsSortQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GoodsSortDAL.Count(0, parm,0);
//数据库查询
list = GoodsSortDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GoodsSortDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GoodsSortDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GoodsSortDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GoodsSortDALEntity[] list = GoodsSortDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GoodsSortDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GoodsSortDALEntity oldEntity = null;
 if (item.GoodsSortId != null)
 oldEntity = GoodsSortBLL.GetEntity(item.GoodsSortId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GoodsSortDALEntity[] input)
{
DataTable result = new DataTable();
List<GoodsSortDALEntity> list = new List<GoodsSortDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GoodsSortDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("商品分类名字", typeof(string));
result.Columns.Add("标签", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (GoodsSortDALEntity item in list)
{
result.Rows.Add(
item.SortName,
item.Laber,
