...
{
public static partial class MzqOrderBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(MzqOrderDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqOrderDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(MzqOrderDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqOrderDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(MzqOrderDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.BillId.ToStr()))
return ret;
ret = MzqOrderDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static MzqOrderDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = MzqOrderDAL.Select(0, id,0);
////添加附加按钮代码
...
public static MzqOrderDALEntity GetCustomCode( MzqOrderDALEntity entity)
{
...
public static List<MzqOrderDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<MzqOrderDALEntity>();
count = 0;
//查询实体赋值
var parm = new MzqOrderQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = MzqOrderDAL.Count(0, parm,0);
//数据库查询
list = MzqOrderDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<MzqOrderDALEntity>();
for (int i = 0; i < list.Count; i++)
{
MzqOrderDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<MzqOrderDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = MzqOrderDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("BillId", YaohuaOrderByType.Desc);
//数据库查询
list = MzqOrderDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 MzqOrderDALEntity[] list = MzqOrderDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (MzqOrderDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 MzqOrderDALEntity oldEntity = null;
 if (item.BillId != null)
 oldEntity = MzqOrderBLL.GetEntity(item.BillId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( MzqOrderDALEntity[] input)
{
DataTable result = new DataTable();
List<MzqOrderDALEntity> list = new List<MzqOrderDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
MzqOrderDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("商品ID", typeof(string));
result.Columns.Add("结算金额", typeof(decimal));
result.Columns.Add("运费", typeof(decimal));
result.Columns.Add("结算时间", typeof(DateTime));
result.Columns.Add("结算状态", typeof(string));
result.Columns.Add("收货地址ID", typeof(string));
result.Columns.Add("用户ID", typeof(string));
result.Columns.Add("交易银行", typeof(string));
result.Columns.Add("银行卡号", typeof(string));
result.Columns.Add("付款方式", typeof(string));
result.Columns.Add("商家备注", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (MzqOrderDALEntity item in list)
{
result.Rows.Add(
item.GoodsId,
