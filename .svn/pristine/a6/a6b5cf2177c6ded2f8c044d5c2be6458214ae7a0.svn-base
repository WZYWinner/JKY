<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="LxcShoppingcar_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.LxcShoppingcarModify" %>
...
<a class="btn btn-primary" href="javascript:window.location.href='LxcShoppingcar_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品名称：</span><asp:TextBox ID="tbLxcShoppingcarName" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td><td  title ="" colspan ="3"><span class="col_name">商品价格：</span><asp:TextBox ID="tbLxcShoppingcarPrice" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品图片：</span><input type="hidden" name="hidden_fileLxcShoppingcarPic" class=""/><UC:UC_PluploadSingle PageType ="可写" ID="fileLxcShoppingcarPic" runat="server" /><b></b></td><td  title ="" colspan ="3"><span class="col_name">商品数量：</span><asp:TextBox ID="tbLxcShoppingcarNum" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td> </tr>
</tbody>
</table>
</div>
</div>
