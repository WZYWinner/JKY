...
<div class="btn_left" style="overflow: hidden; float: left; margin-left: 30px"><a class="btn btn-primary" href="javascript:window.location.href='SysConfig_List.aspx?&id=<%=HttpUtility.UrlEncode(Request["id"])%>&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>'" name="del">返回</a></div>
</small>
</div>
