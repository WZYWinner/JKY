...
{
public static partial class SysMenuBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(SysMenuDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysMenuDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(SysMenuDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysMenuDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(SysMenuDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.MenuId.ToStr()))
return ret;
ret = SysMenuDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static SysMenuDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = SysMenuDAL.Select(0, id,0);
////添加附加按钮代码
...
public static SysMenuDALEntity GetCustomCode( SysMenuDALEntity entity)
{
...
public static List<SysMenuDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<SysMenuDALEntity>();
count = 0;
//查询实体赋值
var parm = new SysMenuQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = SysMenuDAL.Count(0, parm,0);
//数据库查询
list = SysMenuDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<SysMenuDALEntity>();
for (int i = 0; i < list.Count; i++)
{
SysMenuDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<SysMenuDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = SysMenuDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("MenuId", YaohuaOrderByType.Desc);
//数据库查询
list = SysMenuDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 SysMenuDALEntity[] list = SysMenuDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (SysMenuDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 SysMenuDALEntity oldEntity = null;
 if (item.MenuId != null)
 oldEntity = SysMenuBLL.GetEntity(item.MenuId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( SysMenuDALEntity[] input)
{
DataTable result = new DataTable();
List<SysMenuDALEntity> list = new List<SysMenuDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
SysMenuDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("中文名", typeof(string));
result.Columns.Add("权重", typeof(int));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (SysMenuDALEntity item in list)
{
result.Rows.Add(
item.CnName,
item.PowerIndex,
