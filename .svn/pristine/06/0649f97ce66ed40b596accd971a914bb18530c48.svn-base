<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoGoods_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.GuoGoodsDetail" %>
...
<h4 class="txt_title" style="float: left">查看商品</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="GuoGoods_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="商品名" colspan ="3"><span class="col_name">商品名：</span><%=HttpUtility.HtmlEncode(entity.GoodsName.ToStr()) %></td><td  title ="商品价格" colspan ="3"><span class="col_name">商品价格：</span><%=HttpUtility.HtmlEncode(entity.GoodsPrice.ToStr()) %></td> </tr>
<tr> <td  title ="商品数量" colspan ="2"><span class="col_name">商品数量：</span><%=HttpUtility.HtmlEncode(entity.GoodsNum.ToStr()) %></td><td  title ="商品图" colspan ="2"><span class="col_name">商品图：</span><%=HttpUtility.HtmlEncode(entity.GoodsImg.ToStr()) %></td><td  title ="标签" colspan ="2"><span class="col_name">标签：</span><%=HttpUtility.HtmlEncode(entity.Laber.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
