<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MzqShoppingcar_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.MzqShoppingcarDetail" %>
...
<h4 class="txt_title" style="float: left">查看购物车</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="MzqShoppingcar_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品ID：</span><%=HttpUtility.HtmlEncode(entity.MzqGoodsId.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">商品名称：</span><%=HttpUtility.HtmlEncode(entity.MzqShoppingcarName.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">商品价格：</span><%=HttpUtility.HtmlEncode(entity.MzqShoppingcarPrice.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">商品图片：</span><input type="hidden" name="hidden_fileMzqShoppingcarPic" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileMzqShoppingcarPic" runat="server" /></td><td  title ="" colspan ="2"><span class="col_name">商品数量：</span><%=HttpUtility.HtmlEncode(entity.GoodsNumber.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="6"><span class="col_name">商品选择状态：</span><%=HttpUtility.HtmlEncode(entity.CheckedSate.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
