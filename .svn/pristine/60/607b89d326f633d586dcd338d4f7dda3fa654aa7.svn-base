<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="LanOrderList_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.LanOrderListModify" %>
...
<h4 class="txt_title" style="float: left">编辑LAN订单</h4>
...
<a class="btn btn-primary" href="javascript:window.location.href='LanOrderList_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a>
