<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoMerchant_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.GsfAdmin.GuoMerchantDetail" %>
...
<h4 class="txt_title" style="float: left">查看商家</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="GuoMerchant_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="商家名字" colspan ="3"><span class="col_name">商家名字：</span><%=HttpUtility.HtmlEncode(entity.MerchantName.ToStr()) %></td><td  title ="商家图片" colspan ="3"><span class="col_name">商家图片：</span><%=HttpUtility.HtmlEncode(entity.MerchantImg.ToStr()) %></td> </tr>
<tr> <td  title ="商家配送" colspan ="3"><span class="col_name">商家配送：</span><%=HttpUtility.HtmlEncode(entity.MerchantDelivery.ToStr()) %></td><td  title ="标签" colspan ="3"><span class="col_name">标签：</span><%=HttpUtility.HtmlEncode(entity.Laber.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
