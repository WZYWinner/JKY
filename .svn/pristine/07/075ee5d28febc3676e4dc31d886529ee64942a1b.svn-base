<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsClassify_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.GoodsClassifyDetail" %>
...
<h4 class="txt_title" style="float: left">查看商品分类</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="GoodsClassify_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="商品分类ID" colspan ="2"><span class="col_name">商品分类ID：</span><%=HttpUtility.HtmlEncode(entity.GoodsSortId.ToStr()) %></td><td  title ="商品分类名" colspan ="2"><span class="col_name">商品分类名：</span><%=HttpUtility.HtmlEncode(entity.SortName.ToStr()) %></td><td  title ="商品分类ID" colspan ="2"><span class="col_name">商品分类ID：</span><%=HttpUtility.HtmlEncode(entity.MerchantListId.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
