<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoUser_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.GsfAdmin.GuoUserDetail" %>
...
<h4 class="txt_title" style="float: left">查看郭用户</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="GuoUser_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="用户名" colspan ="3"><span class="col_name">用户名：</span><%=HttpUtility.HtmlEncode(entity.UserName.ToStr()) %></td><td  title ="密码" colspan ="3"><span class="col_name">密码：</span><%=HttpUtility.HtmlEncode(entity.Password.ToStr()) %></td> </tr>
<tr> <td  title ="电话" colspan ="3"><span class="col_name">电话：</span><%=HttpUtility.HtmlEncode(entity.Telephone.ToStr()) %></td><td  title ="地址" colspan ="3"><span class="col_name">地址：</span><%=HttpUtility.HtmlEncode(entity.Address.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
