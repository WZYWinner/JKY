<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="SysRole_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.SysRoleModify" %>
...
<h4 class="txt_title" style="float: left">编辑后台角色</h4>
...
<a class="btn btn-primary" href="javascript:window.location.href='SysRole_Detail.aspx?&id=<%=HttpUtility.UrlEncode(Request["id"])%>&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>'" name="del">返回</a>
...
<tbody>
<tr> <td  title ="角色名" colspan ="3"><span class="col_name">角色名：</span><asp:TextBox ID="tbRoleName" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td><td  title ="后台类型" colspan ="3"><span class="col_name">后台类型：</span><asp:DropDownList ID="tbSysType" runat="server" CssClass=""></asp:DropDownList><b></b></td> </tr>
<tr> <td  title ="有权功能列表" colspan ="6"><span class="col_name">有权功能列表：</span><asp:CheckBoxList ID="tbUserPowerList" CssClass="input_ck" TextAlign="Right" RepeatLayout="Table" RepeatColumns="2" RepeatDirection="Horizontal" runat="server" ></asp:CheckBoxList><b></b></td> </tr>
<tr>  </tr>
</tbody>
</table>
</div>
