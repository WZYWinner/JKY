...
{
public static partial class MerchantListBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(MerchantListDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MerchantListDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(MerchantListDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MerchantListDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = MerchantListDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(MerchantListDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.MerchantListId.ToStr()))
return ret;
ret = MerchantListDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static MerchantListDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = MerchantListDAL.Select(0, id,0);
////添加附加按钮代码
...
public static MerchantListDALEntity GetCustomCode( MerchantListDALEntity entity)
{
...
public static List<MerchantListDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<MerchantListDALEntity>();
count = 0;
//查询实体赋值
var parm = new MerchantListQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = MerchantListDAL.Count(0, parm,0);
//数据库查询
list = MerchantListDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<MerchantListDALEntity>();
for (int i = 0; i < list.Count; i++)
{
MerchantListDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<MerchantListDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = MerchantListDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("MerchantListId", YaohuaOrderByType.Desc);
//数据库查询
list = MerchantListDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 MerchantListDALEntity[] list = MerchantListDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (MerchantListDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 MerchantListDALEntity oldEntity = null;
 if (item.MerchantListId != null)
 oldEntity = MerchantListBLL.GetEntity(item.MerchantListId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( MerchantListDALEntity[] input)
{
DataTable result = new DataTable();
List<MerchantListDALEntity> list = new List<MerchantListDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
MerchantListDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("商家ID", typeof(string));
result.Columns.Add("商家名称", typeof(string));
result.Columns.Add("商家LOGO", typeof(string));
result.Columns.Add("配送时间", typeof(string));
result.Columns.Add("配送距离", typeof(string));
result.Columns.Add("起送价格", typeof(string));
result.Columns.Add("基础运费", typeof(string));
result.Columns.Add("商家分类ID", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (MerchantListDALEntity item in list)
{
result.Rows.Add(
item.MerchantListId,
item.MerchantName,
