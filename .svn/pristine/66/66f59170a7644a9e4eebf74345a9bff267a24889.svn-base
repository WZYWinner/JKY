<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="LanSecondMenu_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.LanSecondMenuModify" %>
...
<h4 class="txt_title" style="float: left">编辑菜单明细</h4>
...
<a class="btn btn-primary" href="javascript:window.location.href='LanSecondMenu_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a>
