...
{
public static partial class LxcCarouselBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(LxcCarouselDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcCarouselDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(LxcCarouselDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcCarouselDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = LxcCarouselDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(LxcCarouselDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.LxcCarouselId.ToStr()))
return ret;
ret = LxcCarouselDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static LxcCarouselDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = LxcCarouselDAL.Select(0, id,0);
////添加附加按钮代码
...
public static LxcCarouselDALEntity GetCustomCode( LxcCarouselDALEntity entity)
{
...
public static List<LxcCarouselDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<LxcCarouselDALEntity>();
count = 0;
//查询实体赋值
var parm = new LxcCarouselQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = LxcCarouselDAL.Count(0, parm,0);
//数据库查询
list = LxcCarouselDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<LxcCarouselDALEntity>();
for (int i = 0; i < list.Count; i++)
{
LxcCarouselDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<LxcCarouselDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = LxcCarouselDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("LxcCarouselId", YaohuaOrderByType.Desc);
//数据库查询
list = LxcCarouselDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 LxcCarouselDALEntity[] list = LxcCarouselDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (LxcCarouselDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 LxcCarouselDALEntity oldEntity = null;
 if (item.LxcCarouselId != null)
 oldEntity = LxcCarouselBLL.GetEntity(item.LxcCarouselId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( LxcCarouselDALEntity[] input)
{
DataTable result = new DataTable();
List<LxcCarouselDALEntity> list = new List<LxcCarouselDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
LxcCarouselDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("轮播图路径", typeof(string));
result.Columns.Add("轮播图名称", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (LxcCarouselDALEntity item in list)
{
result.Rows.Add(
item.LxcCarouselUrl,
item.LxcCarouselName,
"");
i++;
}
return result;
