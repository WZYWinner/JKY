<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LxcShoppingcar_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LxcShoppingcarDetail" %>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LxcShoppingcar_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品名称：</span><%=HttpUtility.HtmlEncode(entity.LxcShoppingcarName.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">商品价格：</span><%=HttpUtility.HtmlEncode(entity.LxcShoppingcarPrice.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品图片：</span><input type="hidden" name="hidden_fileLxcShoppingcarPic" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileLxcShoppingcarPic" runat="server" /></td><td  title ="" colspan ="3"><span class="col_name">商品数量：</span><%=HttpUtility.HtmlEncode(entity.LxcShoppingcarNum.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
