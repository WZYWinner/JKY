<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysRoleList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysRoleList" ValidateRequest="false" %>
...
// 绑定事件
url='SysRole_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">角色名</th>
<th style="    min-width: 80px;">后台类型</th>
<th style="    min-width: 80px;">有权功能列表</th>
<th style="    min-width: 80px;">修改人员名称</th>
<th style="    min-width: 80px;">修改时间</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("RoleId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("RoleId") %>' />
 </td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("RoleName").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("SysType").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("UserPowerList").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("EditUserName").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="SysRole_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
