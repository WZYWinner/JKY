...
{
public static partial class WangLoserBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(WangLoserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = WangLoserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(WangLoserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = WangLoserDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = WangLoserDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(WangLoserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.WangLoserId.ToStr()))
return ret;
ret = WangLoserDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static WangLoserDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = WangLoserDAL.Select(0, id,0);
////添加附加按钮代码
...
public static WangLoserDALEntity GetCustomCode( WangLoserDALEntity entity)
{
...
public static List<WangLoserDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<WangLoserDALEntity>();
count = 0;
//查询实体赋值
var parm = new WangLoserQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = WangLoserDAL.Count(0, parm,0);
//数据库查询
list = WangLoserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<WangLoserDALEntity>();
for (int i = 0; i < list.Count; i++)
{
WangLoserDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<WangLoserDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = WangLoserDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("WangLoserId", YaohuaOrderByType.Desc);
//数据库查询
list = WangLoserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 WangLoserDALEntity[] list = WangLoserDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (WangLoserDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 WangLoserDALEntity oldEntity = null;
 if (item.WangLoserId != null)
 oldEntity = WangLoserBLL.GetEntity(item.WangLoserId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( WangLoserDALEntity[] input)
{
DataTable result = new DataTable();
List<WangLoserDALEntity> list = new List<WangLoserDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
WangLoserDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
int i = 0;
////填充DataTable的数据
foreach (WangLoserDALEntity item in list)
{
result.Rows.Add(
item.WangLoserId,
item.WangLoserName,
item.WangLoserPassword,
item.WangLoserAddress,
item.WangLoserTel,
"");
i++;
}
