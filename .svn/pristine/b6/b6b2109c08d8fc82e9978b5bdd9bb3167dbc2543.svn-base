...
{
public static partial class GuoAddressBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GuoAddressDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoAddressDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GuoAddressDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoAddressDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = GuoAddressDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GuoAddressDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.GuoAddressId.ToStr()))
return ret;
ret = GuoAddressDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GuoAddressDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GuoAddressDAL.Select(0, id,0);
////添加附加按钮代码
...
public static GuoAddressDALEntity GetCustomCode( GuoAddressDALEntity entity)
{
...
public static List<GuoAddressDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GuoAddressDALEntity>();
count = 0;
//查询实体赋值
var parm = new GuoAddressQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GuoAddressDAL.Count(0, parm,0);
//数据库查询
list = GuoAddressDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GuoAddressDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GuoAddressDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GuoAddressDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = GuoAddressDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("GuoAddressId", YaohuaOrderByType.Desc);
//数据库查询
list = GuoAddressDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GuoAddressDALEntity[] list = GuoAddressDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GuoAddressDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GuoAddressDALEntity oldEntity = null;
 if (item.GuoAddressId != null)
 oldEntity = GuoAddressBLL.GetEntity(item.GuoAddressId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GuoAddressDALEntity[] input)
{
DataTable result = new DataTable();
List<GuoAddressDALEntity> list = new List<GuoAddressDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GuoAddressDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("省份", typeof(string));
result.Columns.Add("地址", typeof(string));
result.Columns.Add("门牌号", typeof(string));
...
int i = 0;
////填充DataTable的数据
foreach (GuoAddressDALEntity item in list)
{
result.Rows.Add(
item.Province,
item.Address,
