<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MerchantClassifyList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.MerchantClassifyList" ValidateRequest="false" %>
...
// 绑定事件
url='MerchantClassify_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">商家分类名</th>
<th style="    min-width: 80px;">商家分类LOGO</th>
 </tr>
 </thead>
 <tbody>
 <wc:Repeater ID="UiDataList" runat="server">
 <ItemTemplate>
 <tr dateid="<%#Eval("MerchantSortId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("MerchantSortId") %>' />
 </td>
 ...
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("SortName").ToStrCn()) %> </span></td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("SortImg").ToStrCn()) %> </span></td>
 </tr>
 </ItemTemplate>
 <EmptyDataTemplate>
 <!--**** 没有数据的情况 ****-->
...
<a class="btn btn-primary" href="MerchantClassify_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
