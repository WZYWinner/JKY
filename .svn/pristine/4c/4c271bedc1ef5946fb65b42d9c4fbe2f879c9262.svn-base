<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="LanOrderGood_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.LanOrderGoodModify" %>
<%@ Import Namespace="Yaohuasoft.Framework.Library" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
<title>修改LAN订单明细 | </title>
...
<h4 class="txt_title" style="float: left">编辑LAN订单明细</h4>
...
<a class="btn btn-primary" href="javascript:window.location.href='LanOrderGood_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a>
...
<tbody>
<tr>  </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">商品名称：</span><asp:TextBox ID="tbGoodsMsg" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td><td  title ="" colspan ="2"><span class="col_name">商品展示图标：</span><input type="hidden" name="hidden_fileGoodsShowimg" class=""/><UC:UC_PluploadSingle PageType ="可写" ID="fileGoodsShowimg" runat="server" /><b></b></td><td  title ="" colspan ="2"><span class="col_name">商品单价：</span><asp:TextBox ID="tbGoodsPrice" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品数量：</span><asp:TextBox ID="tbLanOrderGoodNum" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td><td  title ="" colspan ="3"><span class="col_name">当前商品总价：</span><asp:TextBox ID="tbLanOrderGoodsSumprice" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td> </tr>
</tbody>
</table>
</div>
</div>
