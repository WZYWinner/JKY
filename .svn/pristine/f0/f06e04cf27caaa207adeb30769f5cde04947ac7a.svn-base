<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsClassifyList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.GoodsClassifyList" ValidateRequest="false" %>
...
// 绑定事件
url='GoodsClassify_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">商品分类名</th>
<th style="    min-width: 80px;">商品分类ID</th>
 </tr>
 </thead>
 <tbody>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("GoodsSortId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("GoodsSortId") %>' />
 </td>
...
<a class="btn btn-primary" href="GoodsClassify_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
