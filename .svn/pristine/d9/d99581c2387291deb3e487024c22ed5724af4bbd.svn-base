<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsSort_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.GsfAdmin.GoodsSortDetail" %>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="GoodsSort_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="商品分类名字" colspan ="3"><span class="col_name">商品分类名字：</span><%=HttpUtility.HtmlEncode(entity.SortName.ToStr()) %></td><td  title ="标签" colspan ="3"><span class="col_name">标签：</span><%=HttpUtility.HtmlEncode(entity.Laber.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
