<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MerchantSortList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.MerchantSortList" ValidateRequest="false" %>
...
// 绑定事件
url='MerchantSort_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">分类名字</th>
<th style="    min-width: 80px;">分类图片</th>
 </tr>
 </thead>
 <tbody>
...
<a class="btn btn-primary" href="MerchantSort_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
