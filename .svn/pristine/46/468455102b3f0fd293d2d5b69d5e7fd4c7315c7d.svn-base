<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="WangLoser_Upload.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.WangLoserUpload" %>
...
<h4 class="txt_title" style="float: left">导入王用户</h4>
<div class="btn_left" style="overflow: hidden; float: left; margin-left: 30px"><a class="btn btn-primary" href="javascript:window.location.href='WangLoser_List.aspx?&id=<%=HttpUtility.UrlEncode(Request["id"])%>&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>'" name="del">返回</a></div>
</small>
</div>
