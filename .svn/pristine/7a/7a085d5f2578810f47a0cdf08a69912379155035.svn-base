<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LanOrderGood_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LanOrderGoodDetail" %>
<%@ Import Namespace="Yaohuasoft.Framework.Library" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
<title>LAN订单明细详情 | </title>
...
<h4 class="txt_title" style="float: left">查看LAN订单明细</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LanOrderGood_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr>  </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">商品名称：</span><%=HttpUtility.HtmlEncode(entity.GoodsMsg.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">商品展示图标：</span><input type="hidden" name="hidden_fileGoodsShowimg" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileGoodsShowimg" runat="server" /></td><td  title ="" colspan ="2"><span class="col_name">商品单价：</span><%=HttpUtility.HtmlEncode(entity.GoodsPrice.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">商品数量：</span><%=HttpUtility.HtmlEncode(entity.LanOrderGoodNum.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">当前商品总价：</span><%=HttpUtility.HtmlEncode(entity.LanOrderGoodsSumprice.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
