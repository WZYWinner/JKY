<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MzqAddress_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.MzqAddressDetail" %>
...
<h4 class="txt_title" style="float: left">查看收货地址</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="MzqAddress_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">用户ID：</span><%=HttpUtility.HtmlEncode(entity.UserId.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">收货人：</span><%=HttpUtility.HtmlEncode(entity.ConsigneeInfo.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">收货区域：</span><%=HttpUtility.HtmlEncode(entity.ConsigneeRegion.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">地区：</span><%=HttpUtility.HtmlEncode(entity.RegionInfo.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">详细地址：</span><%=HttpUtility.HtmlEncode(entity.ReceivingAddress.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="3"><span class="col_name">手机号码：</span><%=HttpUtility.HtmlEncode(entity.Telephone.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">地址状态：</span><%=HttpUtility.HtmlEncode(entity.State.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
