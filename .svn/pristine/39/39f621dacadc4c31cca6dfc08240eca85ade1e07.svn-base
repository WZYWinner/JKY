...
{
public static partial class SysRoleBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(SysRoleDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysRoleDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(SysRoleDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysRoleDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(SysRoleDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.RoleId.ToStr()))
return ret;
ret = SysRoleDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static SysRoleDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = SysRoleDAL.Select(0, id,0);
////添加附加按钮代码
...
public static SysRoleDALEntity GetCustomCode( SysRoleDALEntity entity)
{
...
public static List<SysRoleDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<SysRoleDALEntity>();
count = 0;
//查询实体赋值
var parm = new SysRoleQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = SysRoleDAL.Count(0, parm,0);
//数据库查询
list = SysRoleDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<SysRoleDALEntity>();
for (int i = 0; i < list.Count; i++)
{
SysRoleDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<SysRoleDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = SysRoleDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("RoleId", YaohuaOrderByType.Desc);
//数据库查询
list = SysRoleDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 SysRoleDALEntity[] list = SysRoleDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (SysRoleDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 item.SysType=SysConfigService.GetKey(corpId,"SYS_TYPE",item.SysType);
 SysRoleDALEntity oldEntity = null;
 if (item.RoleId != null)
 oldEntity = SysRoleBLL.GetEntity(item.RoleId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( SysRoleDALEntity[] input)
{
DataTable result = new DataTable();
List<SysRoleDALEntity> list = new List<SysRoleDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
SysRoleDALEntity entity = item.Clone();
...
result.Columns.Add("有权功能列表", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (SysRoleDALEntity item in list)
{
result.Rows.Add(
item.RoleName,
item.SysType,
