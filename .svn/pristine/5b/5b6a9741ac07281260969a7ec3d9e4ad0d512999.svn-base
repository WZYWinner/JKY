...
{
public static partial class ActivityBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(ActivityDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = ActivityDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(ActivityDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = ActivityDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(ActivityDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.Id.ToStr()))
return ret;
ret = ActivityDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static ActivityDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = ActivityDAL.Select(0, id,0);
////添加附加按钮代码
...
public static ActivityDALEntity GetCustomCode( ActivityDALEntity entity)
{
...
public static List<ActivityDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<ActivityDALEntity>();
count = 0;
//查询实体赋值
var parm = new ActivityQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = ActivityDAL.Count(0, parm,0);
//数据库查询
list = ActivityDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<ActivityDALEntity>();
for (int i = 0; i < list.Count; i++)
{
ActivityDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<ActivityDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 ActivityDALEntity[] list = ActivityDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (ActivityDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 ActivityDALEntity oldEntity = null;
 if (item.Id != null)
 oldEntity = ActivityBLL.GetEntity(item.Id.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( ActivityDALEntity[] input)
{
DataTable result = new DataTable();
List<ActivityDALEntity> list = new List<ActivityDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
ActivityDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("活动标题", typeof(string));
result.Columns.Add("卖家", typeof(string));
result.Columns.Add("积分奖励", typeof(int));
result.Columns.Add("活动图片", typeof(string));
result.Columns.Add("活动详细说明", typeof(string));
result.Columns.Add("活动时长", typeof(int));
result.Columns.Add("活动开始时间", typeof(DateTime));
result.Columns.Add("活动结束时间", typeof(DateTime));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (ActivityDALEntity item in list)
{
result.Rows.Add(
item.Title,
item.Seller,
item.ActivityIntegral,
item.ActivityPic,
item.ActivityDetailed,
item.ActivityLength,
