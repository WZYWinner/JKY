<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WangLoser_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.WangLoserDetail" %>
...
<h4 class="txt_title" style="float: left">查看王用户</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="WangLoser_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="2"><span class="col_name">ID：</span><%=HttpUtility.HtmlEncode(entity.WangLoserId.ToStr()) %></td><td  title ="用户名" colspan ="2"><span class="col_name">用户名：</span><%=HttpUtility.HtmlEncode(entity.WangLoserName.ToStr()) %></td><td  title ="密码" colspan ="2"><span class="col_name">密码：</span><%=HttpUtility.HtmlEncode(entity.WangLoserPassword.ToStr()) %></td> </tr>
<tr> <td  title ="地址" colspan ="3"><span class="col_name">地址：</span><%=HttpUtility.HtmlEncode(entity.WangLoserAddress.ToStr()) %></td><td  title ="手机号码" colspan ="3"><span class="col_name">手机号：</span><%=HttpUtility.HtmlEncode(entity.WangLoserTel.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
