<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CarouselOne_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.CarouselOneDetail" %>
...
<h4 class="txt_title" style="float: left">查看轮播图一</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="CarouselOne_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">卖家名称：</span><%=HttpUtility.HtmlEncode(entity.Seller.ToStr()) %></td><td  title ="" colspan ="3"><span class="col_name">广告图片：</span><input type="hidden" name="hidden_fileAdPic" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileAdPic" runat="server" /></td> </tr>
<tr> <td  title ="" colspan ="2"><span class="col_name">广告文字信息：</span><%=HttpUtility.HtmlEncode(entity.AdText.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">最近修改人员：</span><%=HttpUtility.HtmlEncode(entity.EditUser.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">最近修改时间：</span><%=HttpUtility.HtmlEncode(entity.EditTime.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
