<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="LxcCarousel_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.LxcCarouselModify" %>
...
<h4 class="txt_title" style="float: left">编辑轮播图</h4>
...
<a class="btn btn-primary" href="javascript:window.location.href='LxcCarousel_Detail.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>'" name="del">返回</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">轮播图路径：</span><input type="hidden" name="hidden_fileLxcCarouselUrl" class=""/><UC:UC_PluploadSingle PageType ="可写" ID="fileLxcCarouselUrl" runat="server" /><b></b></td><td  title ="" colspan ="3"><span class="col_name">轮播图名称：</span><asp:TextBox ID="tbLxcCarouselName" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td> </tr>
</tbody>
</table>
</div>
</div>
