...
{
public static partial class SignInprizeBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(SignInprizeDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SignInprizeDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(SignInprizeDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SignInprizeDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = SignInprizeDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(SignInprizeDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.Id.ToStr()))
return ret;
ret = SignInprizeDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static SignInprizeDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = SignInprizeDAL.Select(0, id,0);
////添加附加按钮代码
...
public static SignInprizeDALEntity GetCustomCode( SignInprizeDALEntity entity)
{
...
public static List<SignInprizeDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<SignInprizeDALEntity>();
count = 0;
//查询实体赋值
var parm = new SignInprizeQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = SignInprizeDAL.Count(0, parm,0);
//数据库查询
list = SignInprizeDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<SignInprizeDALEntity>();
for (int i = 0; i < list.Count; i++)
{
SignInprizeDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<SignInprizeDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 SignInprizeDALEntity[] list = SignInprizeDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (SignInprizeDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 SignInprizeDALEntity oldEntity = null;
 if (item.Id != null)
 oldEntity = SignInprizeBLL.GetEntity(item.Id.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( SignInprizeDALEntity[] input)
{
DataTable result = new DataTable();
List<SignInprizeDALEntity> list = new List<SignInprizeDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
SignInprizeDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("用户ID", typeof(string));
result.Columns.Add("积分等级", typeof(int));
result.Columns.Add("总积分", typeof(int));
result.Columns.Add("连续签到天数", typeof(int));
result.Columns.Add("签到总计天数", typeof(int));
result.Columns.Add("积分奖励", typeof(DateTime));
result.Columns.Add("签到时间", typeof(DateTime));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (SignInprizeDALEntity item in list)
{
result.Rows.Add(
item.UserId,
item.SignGrade,
