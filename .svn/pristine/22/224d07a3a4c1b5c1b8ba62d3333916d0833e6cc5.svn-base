<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeBehind="GoodsSort_Modify.aspx.cs" Inherits=" Yaohuasoft.Framework.WebUI.SysAdmin.GoodsSortModify" %>
...
<a class="btn btn-primary" href="javascript:window.location.href='GoodsSort_Detail.aspx?&id=<%=HttpUtility.UrlEncode(Request["id"])%>&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>'" name="del">返回</a>
...
<tbody>
<tr> <td  title ="商品分类名字" colspan ="3"><span class="col_name">商品分类名字：</span><asp:TextBox ID="tbSortName" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td><td  title ="标签" colspan ="3"><span class="col_name">标签：</span><asp:TextBox ID="tbLaber" MaxLength="200" runat="server" CssClass="tbox  " ></asp:TextBox><b></b></td> </tr>
</tbody>
</table>
</div>
</div>
