<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LxcCarousel_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LxcCarouselDetail" %>
...
<h4 class="txt_title" style="float: left">查看轮播图</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LxcCarousel_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="3"><span class="col_name">轮播图路径：</span><input type="hidden" name="hidden_fileLxcCarouselUrl" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileLxcCarouselUrl" runat="server" /></td><td  title ="" colspan ="3"><span class="col_name">轮播图名称：</span><%=HttpUtility.HtmlEncode(entity.LxcCarouselName.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
