<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysCorp_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.SysCorpDetail" %>
...
<h4 class="txt_title" style="float: left">查看客户公司</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="SysCorp_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="客户公司ID" colspan ="2"><span class="col_name">客户公司ID：</span><%=HttpUtility.HtmlEncode(entity.CorpId.ToStr()) %></td><td  title ="" colspan ="2"><span class="col_name">公司单位代码：</span><%=HttpUtility.HtmlEncode(entity.CoreCode.ToStr()) %></td><td  title ="客户公司名" colspan ="2"><span class="col_name">客户公司名：</span><%=HttpUtility.HtmlEncode(entity.CorpName.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
