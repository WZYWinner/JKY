...
{
public static partial class MzqPayBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(MzqPayDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqPayDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(MzqPayDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqPayDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(MzqPayDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.Id.ToStr()))
return ret;
ret = MzqPayDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static MzqPayDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = MzqPayDAL.Select(0, id,0);
////添加附加按钮代码
...
public static MzqPayDALEntity GetCustomCode( MzqPayDALEntity entity)
{
...
public static List<MzqPayDALEntity> GetList( YaohuaDict<string, string> dropList,
...
count = 0;
//查询实体赋值
var parm = new MzqPayQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = MzqPayDAL.Count(0, parm,0);
//数据库查询
list = MzqPayDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<MzqPayDALEntity>();
for (int i = 0; i < list.Count; i++)
{
MzqPayDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<MzqPayDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 MzqPayDALEntity[] list = MzqPayDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (MzqPayDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 MzqPayDALEntity oldEntity = null;
 if (item.Id != null)
 oldEntity = MzqPayBLL.GetEntity(item.Id.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( MzqPayDALEntity[] input)
{
DataTable result = new DataTable();
List<MzqPayDALEntity> list = new List<MzqPayDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
MzqPayDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
result.Columns.Add("订单ID", typeof(string));
...
int i = 0;
////填充DataTable的数据
foreach (MzqPayDALEntity item in list)
{
result.Rows.Add(
item.UserId,
item.GoodsId,
