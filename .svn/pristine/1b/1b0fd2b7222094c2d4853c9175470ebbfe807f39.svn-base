...
{
public static partial class GuoGoodsBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GuoGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoGoodsDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GuoGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoGoodsDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GuoGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.GoodsId.ToStr()))
return ret;
ret = GuoGoodsDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GuoGoodsDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GuoGoodsDAL.Select(0, id,0);
////添加附加按钮代码
...
public static GuoGoodsDALEntity GetCustomCode( GuoGoodsDALEntity entity)
{
...
public static List<GuoGoodsDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GuoGoodsDALEntity>();
count = 0;
//查询实体赋值
var parm = new GuoGoodsQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GuoGoodsDAL.Count(0, parm,0);
//数据库查询
list = GuoGoodsDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GuoGoodsDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GuoGoodsDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GuoGoodsDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GuoGoodsDALEntity[] list = GuoGoodsDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GuoGoodsDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GuoGoodsDALEntity oldEntity = null;
 if (item.GoodsId != null)
 oldEntity = GuoGoodsBLL.GetEntity(item.GoodsId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GuoGoodsDALEntity[] input)
{
DataTable result = new DataTable();
List<GuoGoodsDALEntity> list = new List<GuoGoodsDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GuoGoodsDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
result.Columns.Add("商品图", typeof(string));
result.Columns.Add("标签", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (GuoGoodsDALEntity item in list)
{
result.Rows.Add(
item.GoodsName,
item.GoodsPrice,
