...
namespace Yaohuasoft.Framework.Web
{
    ...
    /// Handler1 的摘要说明
    /// </summary>
    public class HuangqianHandler : BaseHanlder
    {
                    ...
                    case "register":
                        {
                            ...
                            HuangUserQueryParameter UserParm = new HuangUserQueryParameter();
                            UserParm.EqualTo.UserName = name;
                            var user = HuangUserDAL.Select(0, UserParm).FirstOrDefault();
                            //判断获取的用户名是否存在
                            if (user.IsNullOrEmptys())
                            {
                                ...
                                entity.UserRealname = realname;
                                entity.UserSex = sex;
                                entity.UserPhone = phone;
                                entity.UserPwd = pwd;
                                entity.UserQuestion = question;
                                entity.UserQanswer = answer;
                                HuangUserDAL.Merge(0, entity);
                                string sql = SystemConfig.SQL;
