...
{
public static partial class GuoOrderFormBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GuoOrderFormDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoOrderFormDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GuoOrderFormDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoOrderFormDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Delete(int id,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证ID
...
return ret;
//实体赋值获取
ret = GuoOrderFormDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GuoOrderFormDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.OrderId.ToStr()))
return ret;
ret = GuoOrderFormDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GuoOrderFormDALEntity GetEntity(int id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GuoOrderFormDAL.Select(0, id,0);
////添加附加按钮代码
...
return;
var entity = GetEntity(id.ToInt(), null, null, null);
...
public static GuoOrderFormDALEntity GetCustomCode( GuoOrderFormDALEntity entity)
{
...
public static List<GuoOrderFormDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GuoOrderFormDALEntity>();
count = 0;
//查询实体赋值
var parm = new GuoOrderFormQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GuoOrderFormDAL.Count(0, parm,0);
//数据库查询
list = GuoOrderFormDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GuoOrderFormDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GuoOrderFormDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GuoOrderFormDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GuoOrderFormDALEntity[] list = GuoOrderFormDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GuoOrderFormDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GuoOrderFormDALEntity oldEntity = null;
 if (item.OrderId != null)
 oldEntity = GuoOrderFormBLL.GetEntity(item.OrderId.ToInt(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GuoOrderFormDALEntity[] input)
{
DataTable result = new DataTable();
List<GuoOrderFormDALEntity> list = new List<GuoOrderFormDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GuoOrderFormDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
int i = 0;
////填充DataTable的数据
foreach (GuoOrderFormDALEntity item in list)
{
result.Rows.Add(
"");
i++;
