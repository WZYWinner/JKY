<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LiuUserList.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LiuUserList" ValidateRequest="false" %>
...
// 绑定事件
url='LiuUser_Detail.aspx?&id=' + encodeURIComponent($(obj).parent().attr('dateid')) + '&rnd=<%=YaohuaID.GenRandomInt()%>&query=' + encodeURIComponent($('#QueryString').val())+'';
}
...
<th style="    min-width: 80px;">电话</th>
 ...
 <ItemTemplate>
 <tr dateid="<%#Eval("LiuUserId") %>" class="data_list">
 <td class="ck">
 <asp:CheckBox ID="chkSelect" runat="server" />
 <asp:HiddenField ID="hiddenID" runat="server" Value='<%# Eval("LiuUserId") %>' />
 </td>
 <td class="c_point" _url="" _Color="" _Key="" onclick="javascript:pic_list_click(this)" ><span> <%#HttpUtility.HtmlEncode(Eval("LiuName").ToStrCn()) %> </span></td>
...
<a class="btn btn-primary" href="LiuUser_Upload.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>">导入</a>
