...
{
public static partial class LxcShoppingcartBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(LxcShoppingcartDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcShoppingcartDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(LxcShoppingcartDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcShoppingcartDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = LxcShoppingcartDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(LxcShoppingcartDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.LxcShoppingcartId.ToStr()))
return ret;
ret = LxcShoppingcartDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static LxcShoppingcartDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = LxcShoppingcartDAL.Select(0, id,0);
////添加附加按钮代码
...
public static LxcShoppingcartDALEntity GetCustomCode( LxcShoppingcartDALEntity entity)
{
...
public static List<LxcShoppingcartDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<LxcShoppingcartDALEntity>();
count = 0;
//查询实体赋值
var parm = new LxcShoppingcartQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = LxcShoppingcartDAL.Count(0, parm,0);
//数据库查询
list = LxcShoppingcartDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<LxcShoppingcartDALEntity>();
for (int i = 0; i < list.Count; i++)
{
LxcShoppingcartDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<LxcShoppingcartDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = LxcShoppingcartDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("LxcShoppingcartId", YaohuaOrderByType.Desc);
//数据库查询
list = LxcShoppingcartDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 LxcShoppingcartDALEntity[] list = LxcShoppingcartDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (LxcShoppingcartDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 LxcShoppingcartDALEntity oldEntity = null;
 if (item.LxcShoppingcartId != null)
 oldEntity = LxcShoppingcartBLL.GetEntity(item.LxcShoppingcartId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( LxcShoppingcartDALEntity[] input)
{
DataTable result = new DataTable();
List<LxcShoppingcartDALEntity> list = new List<LxcShoppingcartDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
LxcShoppingcartDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("商品名称", typeof(string));
result.Columns.Add("商品价格", typeof(string));
result.Columns.Add("商品图片", typeof(string));
result.Columns.Add("商品数量", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (LxcShoppingcartDALEntity item in list)
{
result.Rows.Add(
item.LxcShoppingcartName,
item.LxcShoppingcartPrice,
item.LxcShoppingcartPic,
item.LxcShoppingcartNum,
"");
i++;
}
return result;
