<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LanSecondMenu_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.LanSecondMenuDetail" %>
...
<h4 class="txt_title" style="float: left">查看菜单明细</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="LanSecondMenu_Modify.aspx?rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="" colspan ="6"><span class="col_name">菜单名称：</span><%=HttpUtility.HtmlEncode(entity.LanSecondMenuName.ToStr()) %></td> </tr>
<tr> <td  title ="" colspan ="6"><span class="col_name">菜单图标：</span><input type="hidden" name="hidden_fileLanSecondMenuImg" class=""/><UC:UC_PluploadSingle PageType ="只读" ID="fileLanSecondMenuImg" runat="server" /></td> </tr>
</tbody>
</table>
</div>
</div>
