...
{
public static partial class GuoUserBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GuoUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GuoUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GuoUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.GuoUserId.ToStr()))
return ret;
ret = GuoUserDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GuoUserDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GuoUserDAL.Select(0, id,0);
////添加附加按钮代码
...
public static GuoUserDALEntity GetCustomCode( GuoUserDALEntity entity)
{
...
public static List<GuoUserDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GuoUserDALEntity>();
count = 0;
//查询实体赋值
var parm = new GuoUserQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GuoUserDAL.Count(0, parm,0);
//数据库查询
list = GuoUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GuoUserDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GuoUserDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GuoUserDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = GuoUserDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("GuoUserId", YaohuaOrderByType.Desc);
//数据库查询
list = GuoUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GuoUserDALEntity[] list = GuoUserDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GuoUserDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GuoUserDALEntity oldEntity = null;
 if (item.GuoUserId != null)
 oldEntity = GuoUserBLL.GetEntity(item.GuoUserId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GuoUserDALEntity[] input)
{
DataTable result = new DataTable();
List<GuoUserDALEntity> list = new List<GuoUserDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GuoUserDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
int i = 0;
////填充DataTable的数据
foreach (GuoUserDALEntity item in list)
{
result.Rows.Add(
item.UserName,
item.Password,
