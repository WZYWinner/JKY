<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GoodsList_Detail.aspx.cs" Inherits="Yaohuasoft.Framework.WebUI.SysAdmin.GoodsListDetail" %>
...
<h4 class="txt_title" style="float: left">查看商品列表</h4>
...
<%--自定义Detail的按钮标签之前--%>
<a class="btn btn-primary" hidefocus="" href="GoodsList_Modify.aspx?&rnd=<%=YaohuaID.GenRandomInt()%>&query=<%=HttpUtility.UrlEncode(Request["query"])%>&id=<%=HttpUtility.UrlEncode(Request["id"])%>" >编辑</a>
...
<tbody>
<tr> <td  title ="商品ID" colspan ="2"><span class="col_name">商品ID：</span><%=HttpUtility.HtmlEncode(entity.GoodsListId.ToStr()) %></td><td  title ="商品名" colspan ="2"><span class="col_name">商品名：</span><%=HttpUtility.HtmlEncode(entity.GoodsName.ToStr()) %></td><td  title ="商品图片" colspan ="2"><span class="col_name">商品图片：</span><%=HttpUtility.HtmlEncode(entity.GoodsLogo.ToStr()) %></td> </tr>
<tr> <td  title ="商品单价" colspan ="2"><span class="col_name">商品单价：</span><%=HttpUtility.HtmlEncode(entity.GoodsPrice.ToStr()) %></td><td  title ="商品销量" colspan ="2"><span class="col_name">商品销量：</span><%=HttpUtility.HtmlEncode(entity.GoodsSales.ToStr()) %></td><td  title ="商品数量" colspan ="2"><span class="col_name">商品数量：</span><%=HttpUtility.HtmlEncode(entity.GoodsSum.ToStr()) %></td> </tr>
<tr> <td  title ="商品分类ID" colspan ="6"><span class="col_name">商品分类ID：</span><%=HttpUtility.HtmlEncode(entity.GoodsSortId.ToStr()) %></td> </tr>
</tbody>
</table>
</div>
</div>
