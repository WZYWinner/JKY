...
{
public static partial class LxcSubmitBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(LxcSubmitDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcSubmitDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(LxcSubmitDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcSubmitDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = LxcSubmitDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(LxcSubmitDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.LxcSubmitId.ToStr()))
return ret;
ret = LxcSubmitDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static LxcSubmitDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = LxcSubmitDAL.Select(0, id,0);
////添加附加按钮代码
...
public static LxcSubmitDALEntity GetCustomCode( LxcSubmitDALEntity entity)
{
...
public static List<LxcSubmitDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<LxcSubmitDALEntity>();
count = 0;
//查询实体赋值
var parm = new LxcSubmitQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = LxcSubmitDAL.Count(0, parm,0);
//数据库查询
list = LxcSubmitDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<LxcSubmitDALEntity>();
for (int i = 0; i < list.Count; i++)
{
LxcSubmitDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<LxcSubmitDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = LxcSubmitDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("LxcSubmitId", YaohuaOrderByType.Desc);
//数据库查询
list = LxcSubmitDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 LxcSubmitDALEntity[] list = LxcSubmitDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (LxcSubmitDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 LxcSubmitDALEntity oldEntity = null;
 if (item.LxcSubmitId != null)
 oldEntity = LxcSubmitBLL.GetEntity(item.LxcSubmitId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( LxcSubmitDALEntity[] input)
{
DataTable result = new DataTable();
List<LxcSubmitDALEntity> list = new List<LxcSubmitDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
LxcSubmitDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
result.Columns.Add("下单用户", typeof(string));
result.Columns.Add("手机号", typeof(string));
result.Columns.Add("下单地址", typeof(string));
result.Columns.Add("下单时间", typeof(DateTime));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (LxcSubmitDALEntity item in list)
{
result.Rows.Add(
item.LxcSubmitName,
item.LxcSubmitPrice,
item.LxcSubmitPic,
item.LxcSubmitNum,
item.LxcSubmitAddressName,
item.LxcSubmitAddressPhone,
item.LxcSubmitAddressAdres,
item.LxcSubmitAddressTime,
"");
i++;
}
return result;
