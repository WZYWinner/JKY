...
{
public static partial class HuangUserBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(HuangUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = HuangUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(HuangUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = HuangUserDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = HuangUserDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(HuangUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.Id.ToStr()))
return ret;
ret = HuangUserDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static HuangUserDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = HuangUserDAL.Select(0, id,0);
////添加附加按钮代码
...
public static HuangUserDALEntity GetCustomCode( HuangUserDALEntity entity)
{
...
public static List<HuangUserDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<HuangUserDALEntity>();
count = 0;
//查询实体赋值
var parm = new HuangUserQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = HuangUserDAL.Count(0, parm,0);
//数据库查询
list = HuangUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<HuangUserDALEntity>();
for (int i = 0; i < list.Count; i++)
{
HuangUserDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<HuangUserDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 HuangUserDALEntity[] list = HuangUserDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (HuangUserDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 HuangUserDALEntity oldEntity = null;
 if (item.Id != null)
 oldEntity = HuangUserBLL.GetEntity(item.Id.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( HuangUserDALEntity[] input)
{
DataTable result = new DataTable();
List<HuangUserDALEntity> list = new List<HuangUserDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
HuangUserDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("昵称", typeof(string));
...
result.Columns.Add("密保问题", typeof(string));
result.Columns.Add("密保答案", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (HuangUserDALEntity item in list)
{
result.Rows.Add(
item.UserName,
item.UserRealname,
