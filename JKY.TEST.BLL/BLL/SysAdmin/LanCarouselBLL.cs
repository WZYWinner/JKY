...
{
public static partial class LanCarouselBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(LanCarouselDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LanCarouselDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(LanCarouselDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LanCarouselDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = LanCarouselDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(LanCarouselDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.LanCarouselId.ToStr()))
return ret;
ret = LanCarouselDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static LanCarouselDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = LanCarouselDAL.Select(0, id,0);
////添加附加按钮代码
...
public static LanCarouselDALEntity GetCustomCode( LanCarouselDALEntity entity)
{
...
public static List<LanCarouselDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<LanCarouselDALEntity>();
count = 0;
//查询实体赋值
var parm = new LanCarouselQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = LanCarouselDAL.Count(0, parm,0);
//数据库查询
list = LanCarouselDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<LanCarouselDALEntity>();
for (int i = 0; i < list.Count; i++)
{
LanCarouselDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<LanCarouselDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = LanCarouselDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("LanCarouselId", YaohuaOrderByType.Desc);
//数据库查询
list = LanCarouselDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 LanCarouselDALEntity[] list = LanCarouselDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (LanCarouselDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 LanCarouselDALEntity oldEntity = null;
 if (item.LanCarouselId != null)
 oldEntity = LanCarouselBLL.GetEntity(item.LanCarouselId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( LanCarouselDALEntity[] input)
{
DataTable result = new DataTable();
List<LanCarouselDALEntity> list = new List<LanCarouselDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
LanCarouselDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("图片", typeof(string));
result.Columns.Add("图片名称", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (LanCarouselDALEntity item in list)
{
result.Rows.Add(
item.LanCarouselImg,
item.LanCarouselName,
"");
i++;
}
return result;
