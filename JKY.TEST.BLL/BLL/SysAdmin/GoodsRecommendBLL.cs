...
{
public static partial class GoodsRecommendBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GoodsRecommendDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GoodsRecommendDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GoodsRecommendDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GoodsRecommendDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = GoodsRecommendDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GoodsRecommendDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.Id.ToStr()))
return ret;
ret = GoodsRecommendDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GoodsRecommendDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GoodsRecommendDAL.Select(0, id,0);
////添加附加按钮代码
...
public static GoodsRecommendDALEntity GetCustomCode( GoodsRecommendDALEntity entity)
{
...
public static List<GoodsRecommendDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GoodsRecommendDALEntity>();
count = 0;
//查询实体赋值
var parm = new GoodsRecommendQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GoodsRecommendDAL.Count(0, parm,0);
//数据库查询
list = GoodsRecommendDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GoodsRecommendDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GoodsRecommendDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GoodsRecommendDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GoodsRecommendDALEntity[] list = GoodsRecommendDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GoodsRecommendDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GoodsRecommendDALEntity oldEntity = null;
 if (item.Id != null)
 oldEntity = GoodsRecommendBLL.GetEntity(item.Id.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GoodsRecommendDALEntity[] input)
{
DataTable result = new DataTable();
List<GoodsRecommendDALEntity> list = new List<GoodsRecommendDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GoodsRecommendDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
result.Columns.Add("推荐时长", typeof(int));
result.Columns.Add("推荐位置", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (GoodsRecommendDALEntity item in list)
{
result.Rows.Add(
item.GoodsId,
item.GoodsName,
