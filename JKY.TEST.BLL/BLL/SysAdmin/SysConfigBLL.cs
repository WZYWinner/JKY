...
{
public static partial class SysConfigBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(SysConfigDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysConfigDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(SysConfigDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysConfigDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = SysConfigDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(SysConfigDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.ConfigId.ToStr()))
return ret;
ret = SysConfigDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static SysConfigDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = SysConfigDAL.Select(0, id,0);
////添加附加按钮代码
...
public static SysConfigDALEntity GetCustomCode( SysConfigDALEntity entity)
{
...
public static List<SysConfigDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<SysConfigDALEntity>();
count = 0;
//查询实体赋值
var parm = new SysConfigQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = SysConfigDAL.Count(0, parm,0);
//数据库查询
list = SysConfigDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<SysConfigDALEntity>();
for (int i = 0; i < list.Count; i++)
{
SysConfigDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<SysConfigDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = SysConfigDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("ConfigId", YaohuaOrderByType.Desc);
//数据库查询
list = SysConfigDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 SysConfigDALEntity[] list = SysConfigDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (SysConfigDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 SysConfigDALEntity oldEntity = null;
 if (item.ConfigId != null)
 oldEntity = SysConfigBLL.GetEntity(item.ConfigId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( SysConfigDALEntity[] input)
{
DataTable result = new DataTable();
List<SysConfigDALEntity> list = new List<SysConfigDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
SysConfigDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("数据类型", typeof(string));
result.Columns.Add("权重", typeof(int));
result.Columns.Add("代码", typeof(string));
result.Columns.Add("名称", typeof(string));
result.Columns.Add("描述", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (SysConfigDALEntity item in list)
{
result.Rows.Add(
item.TypeId,
item.PowerIndex,
