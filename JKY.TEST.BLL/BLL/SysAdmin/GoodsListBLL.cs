...
{
public static partial class GoodsListBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GoodsListDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GoodsListDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GoodsListDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GoodsListDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = GoodsListDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GoodsListDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.GoodsListId.ToStr()))
return ret;
ret = GoodsListDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GoodsListDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GoodsListDAL.Select(0, id,0);
////添加附加按钮代码
...
public static GoodsListDALEntity GetCustomCode( GoodsListDALEntity entity)
{
...
public static List<GoodsListDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GoodsListDALEntity>();
count = 0;
//查询实体赋值
var parm = new GoodsListQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GoodsListDAL.Count(0, parm,0);
//数据库查询
list = GoodsListDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GoodsListDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GoodsListDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GoodsListDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = GoodsListDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("GoodsListId", YaohuaOrderByType.Desc);
//数据库查询
list = GoodsListDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GoodsListDALEntity[] list = GoodsListDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GoodsListDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GoodsListDALEntity oldEntity = null;
 if (item.GoodsListId != null)
 oldEntity = GoodsListBLL.GetEntity(item.GoodsListId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GoodsListDALEntity[] input)
{
DataTable result = new DataTable();
List<GoodsListDALEntity> list = new List<GoodsListDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GoodsListDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
