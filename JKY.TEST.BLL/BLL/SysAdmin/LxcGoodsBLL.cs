...
{
public static partial class LxcGoodsBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(LxcGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcGoodsDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(LxcGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcGoodsDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(LxcGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.LxcGoodsId.ToStr()))
return ret;
ret = LxcGoodsDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static LxcGoodsDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = LxcGoodsDAL.Select(0, id,0);
////添加附加按钮代码
...
public static LxcGoodsDALEntity GetCustomCode( LxcGoodsDALEntity entity)
{
...
public static List<LxcGoodsDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<LxcGoodsDALEntity>();
count = 0;
//查询实体赋值
var parm = new LxcGoodsQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = LxcGoodsDAL.Count(0, parm,0);
//数据库查询
list = LxcGoodsDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<LxcGoodsDALEntity>();
for (int i = 0; i < list.Count; i++)
{
LxcGoodsDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<LxcGoodsDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = LxcGoodsDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("LxcGoodsId", YaohuaOrderByType.Desc);
//数据库查询
list = LxcGoodsDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 LxcGoodsDALEntity[] list = LxcGoodsDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (LxcGoodsDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 LxcGoodsDALEntity oldEntity = null;
 if (item.LxcGoodsId != null)
 oldEntity = LxcGoodsBLL.GetEntity(item.LxcGoodsId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( LxcGoodsDALEntity[] input)
{
DataTable result = new DataTable();
List<LxcGoodsDALEntity> list = new List<LxcGoodsDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
LxcGoodsDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
result.Columns.Add("商品子图片", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (LxcGoodsDALEntity item in list)
{
result.Rows.Add(
item.LxcGoodsName,
item.LxcGoodsPrice,
