...
{
public static partial class MzqAddressBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(MzqAddressDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqAddressDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(MzqAddressDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqAddressDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = MzqAddressDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(MzqAddressDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.AddressId.ToStr()))
return ret;
ret = MzqAddressDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static MzqAddressDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = MzqAddressDAL.Select(0, id,0);
////添加附加按钮代码
...
public static MzqAddressDALEntity GetCustomCode( MzqAddressDALEntity entity)
{
...
public static List<MzqAddressDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<MzqAddressDALEntity>();
count = 0;
//查询实体赋值
var parm = new MzqAddressQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = MzqAddressDAL.Count(0, parm,0);
//数据库查询
list = MzqAddressDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<MzqAddressDALEntity>();
for (int i = 0; i < list.Count; i++)
{
MzqAddressDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<MzqAddressDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = MzqAddressDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("AddressId", YaohuaOrderByType.Desc);
//数据库查询
list = MzqAddressDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 MzqAddressDALEntity[] list = MzqAddressDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (MzqAddressDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 item.RegionInfo=SysConfigService.GetKey(corpId,"REGION_INFO",item.RegionInfo);
 MzqAddressDALEntity oldEntity = null;
 if (item.AddressId != null)
 oldEntity = MzqAddressBLL.GetEntity(item.AddressId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( MzqAddressDALEntity[] input)
{
DataTable result = new DataTable();
List<MzqAddressDALEntity> list = new List<MzqAddressDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
MzqAddressDALEntity entity = item.Clone();
entity.RegionInfo=SysConfigService.GetValue("","REGION_INFO",item.RegionInfo);
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("用户ID", typeof(string));
result.Columns.Add("收货人", typeof(string));
result.Columns.Add("收货区域", typeof(string));
result.Columns.Add("地区", typeof(string));
result.Columns.Add("详细地址", typeof(string));
result.Columns.Add("手机号码", typeof(string));
result.Columns.Add("地址状态", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (MzqAddressDALEntity item in list)
{
result.Rows.Add(
item.UserId,
item.ConsigneeInfo,
