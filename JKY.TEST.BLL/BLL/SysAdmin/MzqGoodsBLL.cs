...
{
public static partial class MzqGoodsBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(MzqGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqGoodsDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(MzqGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqGoodsDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(MzqGoodsDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.MzqGoodsId.ToStr()))
return ret;
ret = MzqGoodsDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static MzqGoodsDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = MzqGoodsDAL.Select(0, id,0);
////添加附加按钮代码
...
public static MzqGoodsDALEntity GetCustomCode( MzqGoodsDALEntity entity)
{
...
public static List<MzqGoodsDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<MzqGoodsDALEntity>();
count = 0;
//查询实体赋值
var parm = new MzqGoodsQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = MzqGoodsDAL.Count(0, parm,0);
//数据库查询
list = MzqGoodsDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<MzqGoodsDALEntity>();
for (int i = 0; i < list.Count; i++)
{
MzqGoodsDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<MzqGoodsDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = MzqGoodsDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("MzqGoodsId", YaohuaOrderByType.Desc);
//数据库查询
list = MzqGoodsDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 MzqGoodsDALEntity[] list = MzqGoodsDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (MzqGoodsDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 MzqGoodsDALEntity oldEntity = null;
 if (item.MzqGoodsId != null)
 oldEntity = MzqGoodsBLL.GetEntity(item.MzqGoodsId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( MzqGoodsDALEntity[] input)
{
DataTable result = new DataTable();
List<MzqGoodsDALEntity> list = new List<MzqGoodsDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
MzqGoodsDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
int i = 0;
////填充DataTable的数据
foreach (MzqGoodsDALEntity item in list)
{
result.Rows.Add(
item.MzqGoodsSeller,
item.MzqGoodsName,
