...
{
public static partial class MzqShoppingcarBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(MzqShoppingcarDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqShoppingcarDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(MzqShoppingcarDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = MzqShoppingcarDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = MzqShoppingcarDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(MzqShoppingcarDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.MzqShoppingcarId.ToStr()))
return ret;
ret = MzqShoppingcarDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static MzqShoppingcarDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = MzqShoppingcarDAL.Select(0, id,0);
////添加附加按钮代码
...
public static MzqShoppingcarDALEntity GetCustomCode( MzqShoppingcarDALEntity entity)
{
...
public static List<MzqShoppingcarDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<MzqShoppingcarDALEntity>();
count = 0;
//查询实体赋值
var parm = new MzqShoppingcarQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = MzqShoppingcarDAL.Count(0, parm,0);
//数据库查询
list = MzqShoppingcarDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<MzqShoppingcarDALEntity>();
for (int i = 0; i < list.Count; i++)
{
MzqShoppingcarDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<MzqShoppingcarDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = MzqShoppingcarDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("MzqShoppingcarId", YaohuaOrderByType.Desc);
//数据库查询
list = MzqShoppingcarDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 MzqShoppingcarDALEntity[] list = MzqShoppingcarDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (MzqShoppingcarDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 MzqShoppingcarDALEntity oldEntity = null;
 if (item.MzqShoppingcarId != null)
 oldEntity = MzqShoppingcarBLL.GetEntity(item.MzqShoppingcarId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( MzqShoppingcarDALEntity[] input)
{
DataTable result = new DataTable();
List<MzqShoppingcarDALEntity> list = new List<MzqShoppingcarDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
MzqShoppingcarDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
result.Columns.Add("商品选择状态", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (MzqShoppingcarDALEntity item in list)
{
result.Rows.Add(
item.MzqGoodsId,
item.MzqShoppingcarName,
item.MzqShoppingcarPrice,
item.MzqShoppingcarPic,
item.GoodsNumber,
item.CheckedSate,
"");
i++;
