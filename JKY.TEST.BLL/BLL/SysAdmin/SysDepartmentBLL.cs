...
{
public static partial class SysDepartmentBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(SysDepartmentDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysDepartmentDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(SysDepartmentDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysDepartmentDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = SysDepartmentDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(SysDepartmentDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.DepartmentId.ToStr()))
return ret;
ret = SysDepartmentDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static SysDepartmentDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = SysDepartmentDAL.Select(0, id,0);
////添加附加按钮代码
...
public static SysDepartmentDALEntity GetCustomCode( SysDepartmentDALEntity entity)
{
...
public static List<SysDepartmentDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<SysDepartmentDALEntity>();
count = 0;
//查询实体赋值
var parm = new SysDepartmentQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = SysDepartmentDAL.Count(0, parm,0);
//数据库查询
list = SysDepartmentDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<SysDepartmentDALEntity>();
for (int i = 0; i < list.Count; i++)
{
SysDepartmentDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<SysDepartmentDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = SysDepartmentDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("DepartmentId", YaohuaOrderByType.Desc);
//数据库查询
list = SysDepartmentDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 SysDepartmentDALEntity[] list = SysDepartmentDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (SysDepartmentDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 item.CorpId=SysCorpService.GetKey(corpId,"CORP_ID",item.CorpId);
 SysDepartmentDALEntity oldEntity = null;
 if (item.DepartmentId != null)
 oldEntity = SysDepartmentBLL.GetEntity(item.DepartmentId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( SysDepartmentDALEntity[] input)
{
DataTable result = new DataTable();
List<SysDepartmentDALEntity> list = new List<SysDepartmentDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
SysDepartmentDALEntity entity = item.Clone();
entity.CorpId=SysCorpService.GetValue("","CORP_ID",item.CorpId);
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("公司", typeof(string));
result.Columns.Add("部门代码", typeof(string));
result.Columns.Add("部门名", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (SysDepartmentDALEntity item in list)
{
result.Rows.Add(
item.CorpId,
item.DepartmentCode,
