...
{
public static partial class SysCorpBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(SysCorpDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysCorpDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(SysCorpDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SysCorpDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(SysCorpDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.CorpId.ToStr()))
return ret;
ret = SysCorpDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static SysCorpDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = SysCorpDAL.Select(0, id,0);
////添加附加按钮代码
...
public static SysCorpDALEntity GetCustomCode( SysCorpDALEntity entity)
{
...
public static List<SysCorpDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<SysCorpDALEntity>();
count = 0;
//查询实体赋值
var parm = new SysCorpQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = SysCorpDAL.Count(0, parm,0);
//数据库查询
list = SysCorpDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<SysCorpDALEntity>();
for (int i = 0; i < list.Count; i++)
{
SysCorpDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<SysCorpDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = SysCorpDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("CorpId", YaohuaOrderByType.Desc);
//数据库查询
list = SysCorpDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 SysCorpDALEntity[] list = SysCorpDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (SysCorpDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 SysCorpDALEntity oldEntity = null;
 if (item.CorpId != null)
 oldEntity = SysCorpBLL.GetEntity(item.CorpId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( SysCorpDALEntity[] input)
{
DataTable result = new DataTable();
List<SysCorpDALEntity> list = new List<SysCorpDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
SysCorpDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("公司单位代码", typeof(string));
result.Columns.Add("客户公司名", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (SysCorpDALEntity item in list)
{
result.Rows.Add(
item.CoreCode,
item.CorpName,
