...
{
public static partial class LanUserBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(LanUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LanUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(LanUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LanUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(LanUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.LanUserId.ToStr()))
return ret;
ret = LanUserDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static LanUserDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = LanUserDAL.Select(0, id,0);
////添加附加按钮代码
...
public static LanUserDALEntity GetCustomCode( LanUserDALEntity entity)
{
...
public static List<LanUserDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<LanUserDALEntity>();
count = 0;
//查询实体赋值
var parm = new LanUserQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = LanUserDAL.Count(0, parm,0);
//数据库查询
list = LanUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<LanUserDALEntity>();
for (int i = 0; i < list.Count; i++)
{
LanUserDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<LanUserDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = LanUserDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("LanUserId", YaohuaOrderByType.Desc);
//数据库查询
list = LanUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 LanUserDALEntity[] list = LanUserDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (LanUserDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 LanUserDALEntity oldEntity = null;
 if (item.LanUserId != null)
 oldEntity = LanUserBLL.GetEntity(item.LanUserId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( LanUserDALEntity[] input)
{
DataTable result = new DataTable();
List<LanUserDALEntity> list = new List<LanUserDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
LanUserDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("用户名", typeof(string));
result.Columns.Add("用户密码", typeof(string));
result.Columns.Add("真实姓名", typeof(string));
result.Columns.Add("联系电话", typeof(string));
result.Columns.Add("头像", typeof(string));
result.Columns.Add("性别", typeof(string));
result.Columns.Add("注册时间", typeof(DateTime));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (LanUserDALEntity item in list)
{
result.Rows.Add(
item.LanUserName,
item.LanUserPwd,
item.LanUserRealname,
item.LanUserPhone,
item.LanUserImg,
item.LanUserSex,
item.LanUserAddtime,
