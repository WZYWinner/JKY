...
{
public static partial class LxcUserBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(LxcUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(LxcUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = LxcUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(LxcUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.LxcUserId.ToStr()))
return ret;
ret = LxcUserDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static LxcUserDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = LxcUserDAL.Select(0, id,0);
////添加附加按钮代码
...
public static LxcUserDALEntity GetCustomCode( LxcUserDALEntity entity)
{
...
public static List<LxcUserDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<LxcUserDALEntity>();
count = 0;
//查询实体赋值
var parm = new LxcUserQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = LxcUserDAL.Count(0, parm,0);
//数据库查询
list = LxcUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<LxcUserDALEntity>();
for (int i = 0; i < list.Count; i++)
{
LxcUserDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<LxcUserDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = LxcUserDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("LxcUserId", YaohuaOrderByType.Desc);
//数据库查询
list = LxcUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 LxcUserDALEntity[] list = LxcUserDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (LxcUserDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 LxcUserDALEntity oldEntity = null;
 if (item.LxcUserId != null)
 oldEntity = LxcUserBLL.GetEntity(item.LxcUserId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( LxcUserDALEntity[] input)
{
DataTable result = new DataTable();
List<LxcUserDALEntity> list = new List<LxcUserDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
LxcUserDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("账号", typeof(string));
result.Columns.Add("密码", typeof(string));
result.Columns.Add("账户注册时间", typeof(DateTime));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (LxcUserDALEntity item in list)
{
result.Rows.Add(
item.UserName,
item.UserPasswd,
