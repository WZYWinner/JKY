...
{
public static partial class SecuritiesCentreBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(SecuritiesCentreDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SecuritiesCentreDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(SecuritiesCentreDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = SecuritiesCentreDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = SecuritiesCentreDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(SecuritiesCentreDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.Id.ToStr()))
return ret;
ret = SecuritiesCentreDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static SecuritiesCentreDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = SecuritiesCentreDAL.Select(0, id,0);
////添加附加按钮代码
...
public static SecuritiesCentreDALEntity GetCustomCode( SecuritiesCentreDALEntity entity)
{
...
public static List<SecuritiesCentreDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<SecuritiesCentreDALEntity>();
count = 0;
//查询实体赋值
var parm = new SecuritiesCentreQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = SecuritiesCentreDAL.Count(0, parm,0);
//数据库查询
list = SecuritiesCentreDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<SecuritiesCentreDALEntity>();
for (int i = 0; i < list.Count; i++)
{
SecuritiesCentreDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<SecuritiesCentreDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 SecuritiesCentreDALEntity[] list = SecuritiesCentreDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (SecuritiesCentreDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 SecuritiesCentreDALEntity oldEntity = null;
 if (item.Id != null)
 oldEntity = SecuritiesCentreBLL.GetEntity(item.Id.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( SecuritiesCentreDALEntity[] input)
{
DataTable result = new DataTable();
List<SecuritiesCentreDALEntity> list = new List<SecuritiesCentreDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
SecuritiesCentreDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("领劵次数", typeof(int));
result.Columns.Add("商品ID", typeof(string));
result.Columns.Add("券图", typeof(string));
result.Columns.Add("卖家", typeof(string));
result.Columns.Add("用户ID", typeof(string));
result.Columns.Add("券信息", typeof(string));
result.Columns.Add("券结束时间", typeof(DateTime));
result.Columns.Add("领劵开始时间", typeof(DateTime));
result.Columns.Add("不可用券", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (SecuritiesCentreDALEntity item in list)
{
result.Rows.Add(
item.Num,
item.GoodsId,
