...
{
public static partial class WangUserBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(WangUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = WangUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(WangUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = WangUserDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Save(WangUserDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.WangUserId.ToStr()))
return ret;
ret = WangUserDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static WangUserDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = WangUserDAL.Select(0, id,0);
////添加附加按钮代码
...
public static WangUserDALEntity GetCustomCode( WangUserDALEntity entity)
{
...
public static List<WangUserDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<WangUserDALEntity>();
count = 0;
//查询实体赋值
var parm = new WangUserQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = WangUserDAL.Count(0, parm,0);
//数据库查询
list = WangUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<WangUserDALEntity>();
for (int i = 0; i < list.Count; i++)
{
WangUserDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<WangUserDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = WangUserDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("WangUserId", YaohuaOrderByType.Desc);
//数据库查询
list = WangUserDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 WangUserDALEntity[] list = WangUserDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (WangUserDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 WangUserDALEntity oldEntity = null;
 if (item.WangUserId != null)
 oldEntity = WangUserBLL.GetEntity(item.WangUserId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( WangUserDALEntity[] input)
{
DataTable result = new DataTable();
List<WangUserDALEntity> list = new List<WangUserDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
WangUserDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
...
int i = 0;
////填充DataTable的数据
foreach (WangUserDALEntity item in list)
{
result.Rows.Add(
item.UserName,
item.Password,
