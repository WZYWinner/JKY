...
{
public static partial class OrderDetailBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(OrderDetailDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = OrderDetailDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(OrderDetailDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = OrderDetailDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = OrderDetailDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(OrderDetailDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.OrderDetailId.ToStr()))
return ret;
ret = OrderDetailDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static OrderDetailDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = OrderDetailDAL.Select(0, id,0);
////添加附加按钮代码
...
public static OrderDetailDALEntity GetCustomCode( OrderDetailDALEntity entity)
{
...
public static List<OrderDetailDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<OrderDetailDALEntity>();
count = 0;
//查询实体赋值
var parm = new OrderDetailQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
////通过主表ID取附表
if (textBox["OrderId"].IsNullOrEmptys()==false)parm.EqualTo.OrderId = textBox["OrderId"].ToString();
//统计查询 分页使用
count = OrderDetailDAL.Count(0, parm,0);
//数据库查询
list = OrderDetailDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<OrderDetailDALEntity>();
for (int i = 0; i < list.Count; i++)
{
OrderDetailDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<OrderDetailDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = OrderDetailDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("OrderDetailId", YaohuaOrderByType.Desc);
//数据库查询
list = OrderDetailDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 OrderDetailDALEntity[] list = OrderDetailDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (OrderDetailDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 OrderDetailDALEntity oldEntity = null;
 if (item.OrderDetailId != null)
 oldEntity = OrderDetailBLL.GetEntity(item.OrderDetailId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( OrderDetailDALEntity[] input)
{
DataTable result = new DataTable();
List<OrderDetailDALEntity> list = new List<OrderDetailDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
OrderDetailDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("订单号", typeof(string));
result.Columns.Add("商品名称", typeof(string));
result.Columns.Add("商品单价", typeof(string));
result.Columns.Add("商品数量", typeof(string));
result.Columns.Add("商品小计", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (OrderDetailDALEntity item in list)
{
result.Rows.Add(
item.OrderId,
item.OrderName,
