...
{
public static partial class GuoMerchantBLL
{
/// <summary>
/// 新增
/// </summary>
/// <returns>执行结果</returns>
public static int Add(GuoMerchantDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoMerchantDAL.Merge(0, entity,0);
////调试用SQL
...
/// </summary>
/// <returns>执行结果</returns>
public static int Merge(GuoMerchantDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
ret = GuoMerchantDAL.Merge(0, entity,0);
////调试用SQL
...
return ret;
//实体赋值获取
ret = GuoMerchantDAL.Delete(0, id,0);
return ret;
}
/// <summary>
/// 修改
/// </summary>
/// <returns>执行结果</returns>
public static int Save(GuoMerchantDALEntity entity,string corpId,string departmentId,string userName)
{
var ret = 0;
//验证名称
if (string.IsNullOrEmpty(entity.MerchantId.ToStr()))
return ret;
ret = GuoMerchantDAL.Update(0, entity,0);
return ret;
}
/// <summary>
/// 根据ID获取单个实体
/// </summary>
public static GuoMerchantDALEntity GetEntity(string id,string corpId,string departmentId,string userName)
{
//验证ID
if (string.IsNullOrEmpty(id.ToStr()))
return null;
//实体获取
var entity = GuoMerchantDAL.Select(0, id,0);
////添加附加按钮代码
...
public static GuoMerchantDALEntity GetCustomCode( GuoMerchantDALEntity entity)
{
...
public static List<GuoMerchantDALEntity> GetList( YaohuaDict<string, string> dropList,
...
{
var list = new List<GuoMerchantDALEntity>();
count = 0;
//查询实体赋值
var parm = new GuoMerchantQueryParameter();
parm.Pager.PageIndex = CurrentPageIndex-1;//数据从索引0开始
parm.Pager.OnePageSize = PageSize;
//统计查询 分页使用
count = GuoMerchantDAL.Count(0, parm,0);
//数据库查询
list = GuoMerchantDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
///////////////////////////////
var list2 = new List<GuoMerchantDALEntity>();
for (int i = 0; i < list.Count; i++)
{
GuoMerchantDALEntity entity = list[i].Clone();
////添加附加按钮代码
entity=GetCustomCode(entity);
list2.Add(entity);
}
...
/// <param name="count">列表统计</param>
/// <returns>项目实体列表</returns>
public static List<GuoMerchantDALEntity> GetList4Export( YaohuaDict<string, string> dropList,
YaohuaDict<string, string> textBox,YaohuaDict<string, string> textBoxInOne, YaohuaDict<string, string> sortList
...
count = GuoMerchantDAL.Count(0, parm,0);
//排序
parm.OrderBy.Add("MerchantId", YaohuaOrderByType.Desc);
//数据库查询
list = GuoMerchantDAL.Select(0, parm,0).ToList();
////调试用SQL
string sql=SystemConfig.SQL;
...
return list2;
 ...
 int AuditCount = 0;
 int RowNum = 1;
 GuoMerchantDALEntity[] list = GuoMerchantDAL.DataTable2Array4CnName(table, int.MaxValue);
 foreach (var item in list)
 {
 RowNum++;
 if (GuoMerchantDALEntity.IsEmpty(item) == true) { RecordImportLog(TaskId, RowNum, "空行"); continue; }
 ////解析数据
 GuoMerchantDALEntity oldEntity = null;
 if (item.MerchantId != null)
 oldEntity = GuoMerchantBLL.GetEntity(item.MerchantId.ToString(), corpId, departmentId, userName);
 switch (ImportType)
 {
...
/// <returns></returns>
static public DataTable EntityList2DataTableCn( GuoMerchantDALEntity[] input)
{
DataTable result = new DataTable();
List<GuoMerchantDALEntity> list = new List<GuoMerchantDALEntity>();
////将LIST列表的ID变成名字
foreach (var item in input)
{
////克隆后加入新列表
GuoMerchantDALEntity entity = item.Clone();
list.Add(entity);
}
////构建DataTable的列
result.Columns.Add("商家名字", typeof(string));
result.Columns.Add("商家图片", typeof(string));
result.Columns.Add("商家配送", typeof(string));
result.Columns.Add("标签", typeof(string));
result.Columns.Add(" ", typeof(string));
int i = 0;
////填充DataTable的数据
foreach (GuoMerchantDALEntity item in list)
{
result.Rows.Add(
item.MerchantName,
item.MerchantImg,
item.MerchantDelivery,
item.Laber,
"");
i++;
}
